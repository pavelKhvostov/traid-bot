"""Скачивает SOLUSDT 1m с Binance Spot за 2020-05-01 → 2026-05-21."""
...
CACHE = ROOT / "data" / "SOLUSDT_1m_vic_vadim.csv"
...
def main() -> None:
    ...
    while cur < END:
        ...
        for attempt in range(8):
            try:
                r = requests.get(BINANCE_URL, params=params, timeout=60)
                r.raise_for_status()
                batch = r.json()
                break
                    ...
                    ValueError) as e:
                wait = 2 ** attempt
                print(f"  retry {attempt+1} after {wait}s ({type(e).__name__})", flush=True)
                time.sleep(wait)
        if batch is None:
            print("  give up after 8 retries"); break
        if not batch:
            break
        rows.extend(batch)
