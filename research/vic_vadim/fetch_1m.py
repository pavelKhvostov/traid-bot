"""Скачивает BTCUSDT 1m с Binance Spot за 2020-05-01 → 2026-05-21.
Для пересчёта maxV в стратегии 12h-фрактал-предсказания на 1m LTF
(по решению пользователя 2026-05-20)."""
...
END = pd.Timestamp("2026-05-21", tz="UTC")
...
def main() -> None:
    if CACHE.exists():
        ...
        last_ot = existing.index.max()
        start = last_ot + pd.Timedelta(minutes=1)
    ...
    while cur < END:
        ...
        time.sleep(0.10)
        if len(rows) % 50000 == 0:
            print(f"  fetched up to {last_close.date()} {last_close.time()}, total {len(rows):,}", flush=True)
