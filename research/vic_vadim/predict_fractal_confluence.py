...
def main() -> None:
    ...
    for tf, freq in LTF_LIST:
        ...
        for f in fvgs:
            ...
            (fvg_short_all if f["dir"] == "SHORT" else fvg_long_all).append(t)
        for e in ifvgs:
    ...
    c2_hh = {
        "FVG (any LTF)":        flags_in_12h(df_12h, fvg_short_all),
        "iFVG (any LTF)":       flags_in_12h(df_12h, ifvg_b2s_all),
        "iFVG (1h-2h only)":    flags_in_12h(df_12h, ifvg_b2s_1h2h),
    }
    c2_ll = {
        "FVG (any LTF)":        flags_in_12h(df_12h, fvg_long_all),
        "iFVG (any LTF)":       flags_in_12h(df_12h, ifvg_s2b_all),
        "iFVG (1h-2h only)":    flags_in_12h(df_12h, ifvg_s2b_1h2h),
    }
