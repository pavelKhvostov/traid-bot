"""Влияние RDRB V1 (зона эффективности, canon) на предсказание HH/LL
фрактала на 12h BTC.

RDRB V1 формула (canon, intersection — узкая зона пересечения фитилей):
  LONG zone:  top    = min(anchor.high, trigger.close)
              bottom = max(trigger.low, anchor.close)
  SHORT zone: top    = min(trigger.high, anchor.close)
...
Сигналы свечой i:
  HH | FT SHORT RDRB V1 на i
  HH | sweep SHORT RDRB V1: high(i)>zone.top AND close(i)<zone.top
  LL | FT LONG RDRB V1 на i
  LL | sweep LONG RDRB V1: low(i)<zone.bottom AND close(i)>zone.bottom
...
def find_rdrb_v2(df_tf: pd.DataFrame, tf_label: str) -> list[dict]:
    ...
    for i in range(2, len(df_tf)):
        z = detect_rdrb(df_tf, i, zone_version="V1")
        if z is None:
            continue
        out.append({
...
def main() -> None:
    ...
    for tf, df_tf in htf_dfs.items():
        ...
        print(f"  {tf}: RDRB V1 LONG={n_long}, SHORT={n_short}, всего {len(z)}")
    print(f"\nвсего RDRB V1 на 5 ТФ: {len(all_z)}")
    ...
    print("\n=== RDRB V1 общее ===")
    report("HH | RDRB V1 ft[i]",       hh, ft_s[valid], base_hh)
    report("HH | RDRB V1 ft[i-1]",     hh, ft_s[valid-1], base_hh)
    report("HH | RDRB V1 sweep[i]",    hh, sw_s[valid], base_hh)
    report("LL | RDRB V1 ft[i]",       ll, ft_l[valid], base_ll)
    report("LL | RDRB V1 ft[i-1]",     ll, ft_l[valid-1], base_ll)
    report("LL | RDRB V1 sweep[i]",    ll, sw_l[valid], base_ll)

    print("\n=== Per-TF RDRB V1 (на свече i) ===")
    for tf, _ in HTF_LIST:
        z_tf = per_tf[tf]
        if not z_tf:
            print(f"\n--- {tf}: 0 RDRB V1, пропускаем ---")
            continue
        ...
        report(f"HH | RDRB V1 ft[i] ({tf} SHORT)",    hh, ft_s_tf[valid], base_hh)
        report(f"HH | RDRB V1 sweep[i] ({tf} SHORT)", hh, sw_s_tf[valid], base_hh)
        report(f"LL | RDRB V1 ft[i] ({tf} LONG)",     ll, ft_l_tf[valid], base_ll)
        report(f"LL | RDRB V1 sweep[i] ({tf} LONG)",  ll, sw_l_tf[valid], base_ll)
