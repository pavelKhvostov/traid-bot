...
def main() -> None:
    ...
    # === Условие 2 — LTF OB, LTF FVG, LTF iFVG ===
    ...
    fvg_short_all: list[pd.Timestamp] = []; fvg_long_all: list[pd.Timestamp] = []
    fvg_short_1h2h: list[pd.Timestamp] = []; fvg_long_1h2h: list[pd.Timestamp] = []
    ifvg_b2s_all: list[pd.Timestamp] = []; ifvg_s2b_all: list[pd.Timestamp] = []
    ifvg_b2s_1h2h: list[pd.Timestamp] = []; ifvg_s2b_1h2h: list[pd.Timestamp] = []

    for tf, freq in LTF_LIST:
        df_tf = compose_ltf(df_15m, freq).sort_index()
        obs = find_ob_zones(df_tf, tf)
        for ob in obs:
        ...
        for f in fvgs:
            ...
            if t.tz is None: t = t.tz_localize("UTC")
            if f["dir"] == "SHORT":
                fvg_short_all.append(t)
                if tf in ("1h", "2h"): fvg_short_1h2h.append(t)
            else:
                fvg_long_all.append(t)
                if tf in ("1h", "2h"): fvg_long_1h2h.append(t)
        ifvgs = find_ifvg_events(df_tf, fvgs)
        for e in ifvgs:
            t = pd.Timestamp(e["event_time"])
            if t.tz is None: t = t.tz_localize("UTC")
            if e["dir_a"] == "LONG" and e["dir_b"] == "SHORT":
                ifvg_b2s_all.append(t)
                if tf in ("1h", "2h"): ifvg_b2s_1h2h.append(t)
            elif e["dir_a"] == "SHORT" and e["dir_b"] == "LONG":
                ifvg_s2b_all.append(t)
                if tf in ("1h", "2h"): ifvg_s2b_1h2h.append(t)

    f_ob_short_any = flags_in_12h(df_12h, ob_short_all)
    f_ob_long_any  = flags_in_12h(df_12h, ob_long_all)
    f_ob_short_12  = flags_in_12h(df_12h, ob_short_1h2h)
    f_ob_long_12   = flags_in_12h(df_12h, ob_long_1h2h)
    f_fvg_short_any = flags_in_12h(df_12h, fvg_short_all)
    f_fvg_long_any  = flags_in_12h(df_12h, fvg_long_all)
    f_fvg_short_12  = flags_in_12h(df_12h, fvg_short_1h2h)
    f_fvg_long_12   = flags_in_12h(df_12h, fvg_long_1h2h)
    f_ifvg_b2s_any = flags_in_12h(df_12h, ifvg_b2s_all)
    f_ifvg_s2b_any = flags_in_12h(df_12h, ifvg_s2b_all)
    f_ifvg_b2s_12  = flags_in_12h(df_12h, ifvg_b2s_1h2h)
    f_ifvg_s2b_12  = flags_in_12h(df_12h, ifvg_s2b_1h2h)

    print(f"\nLTF (any 15m-4h): OB SHORT={f_ob_short_any.sum()}, FVG SHORT={f_fvg_short_any.sum()},"
          f" iFVG b2s={f_ifvg_b2s_any.sum()}")
    print(f"LTF (1h-2h):      OB SHORT={f_ob_short_12.sum()}, FVG SHORT={f_fvg_short_12.sum()},"
          f" iFVG b2s={f_ifvg_b2s_12.sum()}")

    c2_hh = {
        "OB(any) ∩ FVG(any)":           f_ob_short_any & f_fvg_short_any,
        "OB(any) ∩ iFVG(any)":          f_ob_short_any & f_ifvg_b2s_any,
        "OB(any) ∩ (FVG ∪ iFVG)":       f_ob_short_any & (f_fvg_short_any | f_ifvg_b2s_any),
        "OB(1h-2h) ∩ FVG(1h-2h)":       f_ob_short_12 & f_fvg_short_12,
        "OB(1h-2h) ∩ iFVG(1h-2h)":      f_ob_short_12 & f_ifvg_b2s_12,
        "OB(1h-2h) ∩ (FVG ∪ iFVG)1h-2h": f_ob_short_12 & (f_fvg_short_12 | f_ifvg_b2s_12),
    }
    c2_ll = {
        "OB(any) ∩ FVG(any)":           f_ob_long_any & f_fvg_long_any,
        "OB(any) ∩ iFVG(any)":          f_ob_long_any & f_ifvg_s2b_any,
        "OB(any) ∩ (FVG ∪ iFVG)":       f_ob_long_any & (f_fvg_long_any | f_ifvg_s2b_any),
        "OB(1h-2h) ∩ FVG(1h-2h)":       f_ob_long_12 & f_fvg_long_12,
        "OB(1h-2h) ∩ iFVG(1h-2h)":      f_ob_long_12 & f_ifvg_s2b_12,
        "OB(1h-2h) ∩ (FVG ∪ iFVG)1h-2h": f_ob_long_12 & (f_fvg_long_12 | f_ifvg_s2b_12),
    }
