"""Stage 2 на SWEPT: SL варьируется между ob_htf и fvg-entry границами.
...
DAYS_BACK = 1095
SYMBOL = "BTCUSDT"
ENTRY_PCT = 0.80   # best из Stage 1
...
def main() -> None:
    print(f"[INFO] Stage 2 SWEPT: SL varies [ob_htf -> fvg edge], entry=0.80, no_entry=on")
    print()
    ...
    print()
    print("=" * 100)
    print(f"Stage 2: entry_pct={ENTRY_PCT}, TP=TP_const, SL варьируется [ob_htf -> fvg edge]")
    ...
    print(f"  Baseline (sl_pct=0, ob_htf edge): wins={baseline['wins']} losses={baseline['losses']} "
          f"WR={baseline['wr']}% PnL={baseline['pnl_r']}R avg_RR={baseline['avg_rr']}")

    out = Path("signals/optimize_1_1_1_swept_stage2.csv")
