"""SWEPT split на baseline 1.1.4 (default: fvg_variant=v2).

Аналог analyze_1_1_3_ob_swept.py для 1.1.4:
  - macro=FVG (как в 1.1.1)
  - entry-FVG того же ТФ что OB-htf (как в 1.1.3)
  - tf_minutes для fill_scan: 60 если htf_tf=="1h", 120 если "2h"

Сравниваем ALL / SWEPT / NOT-SWEPT на RR=1.0 и RR=2.2 с no_entry=on.
Default config: entry=mid FVG-htf, SL=15% inside top-OB.

Гипотеза: SWEPT работает (как в 1.1.1, потому что macro=FVG), в отличие
от 1.1.2/1.1.3 (macro=OB).
"""
...
def precompute(sig: dict, df_1m: pd.DataFrame) -> dict | None:
    ...
    # 1.1.4: entry-FVG того же ТФ что OB-htf, fvg_tf == htf_tf.
...
def main() -> None:
    print(f"[INFO] Strategy 1.1.4: SWEPT split (default: fvg_variant=v2)")
    print()
    ...
    out = Path("signals/analyze_1_1_4_v2_ob_swept_summary.txt")
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w", encoding="utf-8") as f:
        f.write(f"1.1.4 SWEPT split summary (fvg_variant=v2)\n")
