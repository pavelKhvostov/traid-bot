"""Forensic 1.1.4 — те же индикаторы что 1.1.2 / 1.1.3 / 1.1.7.

Baseline 6.3y BTC, RR=2.2.
Параметры (как делал детектор 1.1.4 by default):
  entry = mid FVG entry (15m/20m)
  SL    = ob_htf.bottom (LONG) / ob_htf.top (SHORT)
  RR    = 2.2

Output: research/1_1_4/optimize/output/forensic_1_1_4_features.csv
"""
...
def main():
    print("=" * 72)
    print(f"  Forensic 1.1.4 — Indicator overlay (6.3y BTC, RR=2.2)")
    print(f"  entry=mid FVG (entry-15m/20m), SL=ob_htf border")
    print("=" * 72)
    ...
    uniq = []
    for s in sigs:
        key = (s["signal_time"], s["direction"], round(s["entry"], 2))
    ...
    csv = OUT_DIR / "forensic_1_1_4_features.csv"
