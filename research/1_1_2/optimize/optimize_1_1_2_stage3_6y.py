"""Stage 3 на 1.1.2: vary RR с фиксированными entry=0.70 и sl_pct=0.35.

Параметры:
  - entry_pct = 0.70 (best Stage 1)
  - sl_pct = 0.35 (между ob_htf edge и fvg entry edge)
      LONG:  SL = ob_htf.bottom + 0.35 × (fvg.bottom - ob_htf.bottom)
      SHORT: SL = ob_htf.top    - 0.35 × (ob_htf.top    - fvg.top)
  - RR ∈ [1.0, 6.0] step 0.1
      TP = entry ± RR × risk
  - no_entry: ON

Прогон: SWEPT-only И ALL — для сравнения.
"""
...
def precompute(sig: dict, df_1m: pd.DataFrame) -> dict | None:
    ...
    # ограничиваем forward 30 днями — иначе OOM на 6.3y (>1M строк per setup)
...
def stage3_grid(cache: list[dict], group_name: str) -> pd.DataFrame:
    rows = []
...
def print_top(df: pd.DataFrame, label: str, top_n: int = 12) -> None:
    print()
    print("=" * 100)
    print(f"Stage 3 — {label}")
    print("=" * 100)
    df_sorted = df.sort_values("pnl_r", ascending=False)
    print(df_sorted.drop(columns=["group"]).head(top_n).to_string(index=False))
...
def main() -> None:
    print(f"[INFO] Stage 3 на 1.1.2: entry={ENTRY_PCT}, sl_pct={SL_PCT}, RR [1..6] step 0.1")
    print()
    ...
    df_swept = stage3_grid(cache_swept, "SWEPT")
    df_all = stage3_grid(cache_all, "ALL")

    print_top(df_swept, "SWEPT")
    print_top(df_all, "ALL")

    print()
    print("=" * 100)
    print("СВОДКА — best RR в каждой группе:")
    print("=" * 100)
    summary = []
    ...
    out = Path("signals/optimize_1_1_2_stage3_6y.csv")
    out.parent.mkdir(parents=True, exist_ok=True)
    pd.concat([df_swept, df_all]).to_csv(out, index=False)
    print(f"\n  saved: {out}")
