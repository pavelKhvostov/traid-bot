"""Stage 1 на 1.1.2 SWEPT: vary entry_pct, SL=ob_htf edge, TP=TP_const, no_entry=on.

Mirror optimize_1_1_1_swept_stage1.py, но на сигналах новой 1.1.2
(OB-4h/6h macro вместо FVG-4h/6h).

Параметры:
  - entry_pct ∈ [0.0, 1.0] step 0.05
  - SL = ob_htf edge (sl_pct = 0)
  - TP = TP_const = price при entry=mid FVG, SL=ob_htf edge, RR=1
  - no_entry: ON
  - SWEPT filter: ON (OB-htf пара сметает min/max двух предыдущих 1h/2h-свечей)
"""
...
def main() -> None:
    print(f"[INFO] Stage 1 на 1.1.2 SWEPT: vary entry_pct, SL=ob_htf edge, TP=TP_const, no_entry=on")
    print()
    ...
    # Dedup по (signal_time, direction, entry) + SWEPT split
    ...
    print(f"  deduped groups: {len(groups)}  SWEPT: {len(swept_reps)}  NOT-SWEPT: {len(not_swept_reps)}")
    ...
    print()
    print("=" * 100)
    print(f"Stage 1: SL=ob_htf edge, TP=TP_const, vary entry_pct in [0..1] step 0.05")
    print("=" * 100)
    rows = []
    ...
    print(f"      wins={best['wins']} losses={best['losses']} no_entry={best['no_entry']} "
          f"WR={best['wr']}% PnL={best['pnl_r']}R avg_RR={best['avg_rr']}")

    out = Path("signals/optimize_1_1_2_swept_stage1.csv")
