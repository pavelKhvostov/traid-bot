"""Бэктест Strategy 1.1.3 на BTCUSDT.

1.1.3 = 1.1.2 с заменой entry FVG-15m/20m на immediate FVG того же ТФ
(1h или 2h), где c0 FVG = cur OB-htf.

Симуляция: limit-вход = середина FVG-htf (1h или 2h). SL = OB_SL_DEPTH inside top-OB.
RR=1.0 и RR=2.2.
"""
...
SYMBOL = "BTCUSDT"
RR_RUNS = [
    (1.0, Path("signals/strategy_1_1_3_3y_RR1.csv")),
    (2.2, Path("signals/strategy_1_1_3_3y_RR2.2.csv")),
]
SL_TOLERANCE = 0.005
...
def simulate_outcome(sig: dict, df_1m: pd.DataFrame, rr_ratio: float) -> dict:
    ...
    # FVG ТФ совпадает с OB-htf: 1h -> 60min, 2h -> 120min
    ...
    base_row = {
        ...
        "ob_macro_top": sig["ob_macro_zone"][1],
        "ob_macro_bottom": sig["ob_macro_zone"][0],
...
def dedupe_signals(rows: list[dict]) -> list[dict]:
    ...
    for group in buckets:
        ...
        macro_times = [r["ob_macro_time"] for r in group]
        macro_tfs = sorted({r["ob_macro_tf"] for r in group})
        ...
        new = {
            ...
            "ob_macro_time": min(macro_times),
            "ob_macro_tf": ",".join(macro_tfs),
            "ob_macro_count": len({t for t in macro_times}),
            ...
            "ob_macro_top": first["ob_macro_top"],
            "ob_macro_bottom": first["ob_macro_bottom"],
...
def main():
    ...
    print(f"[INFO] Strategy 1.1.3 backtest, {SYMBOL}, окно {DAYS_BACK}d, RR={rrs}")
    print()
    ...
    signals = detect_strategy_1_1_3_signals(
    ...
    for rr_ratio, output_path in RR_RUNS:
        ...
        print(f"  raw: {len(rows)}  deduped: {len(deduped)}  схлопнуто: {len(rows) - len(deduped)}")
