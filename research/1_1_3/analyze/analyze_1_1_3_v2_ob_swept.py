"""SWEPT split на baseline 1.1.3 (default: fvg_variant=v2, macro_mode=untouched).

Аналог analyze_1_1_2_ob_swept.py для 1.1.3:
  - Детектор 1.1.3 не использует df_15m/df_20m (entry-FVG того же ТФ что OB-htf).
  - tf_minutes для fill_scan: 60 если htf_tf=="1h", 120 если "2h" (FVG того же ТФ).
  - SWEPT проверка идентична 1.1.1/1.1.2 — на OB-htf prev/cur пара.

Сравниваем ALL / SWEPT / NOT-SWEPT на RR=1.0 и RR=2.2 с no_entry=on.
Default config: entry=mid FVG-htf, SL=15% inside top-OB.
"""
...
def precompute(sig: dict, df_1m: pd.DataFrame) -> dict | None:
    ...
    # 1.1.3: fvg_tf равен htf_tf (entry-FVG того же ТФ что OB-htf).
    # tf_minutes выводим из htf_tf, а не из fvg_tf — это одно и то же значение
    # (1h→60, 2h→120), просто документируем источник правды.
...
def main() -> None:
    print(f"[INFO] Strategy 1.1.3: SWEPT split (default: fvg_variant=v2, macro_mode=untouched)")
    print()
    ...
    raw = detect_strategy_1_1_3_signals(
        ...
        fvg_variant="v2", macro_mode="untouched", verbose=False,
    )
    ...
    out = Path("signals/analyze_1_1_3_v2_ob_swept_summary.txt")
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w", encoding="utf-8") as f:
        f.write(f"1.1.3 SWEPT split summary (fvg_variant=v2, macro_mode=untouched)\n")
