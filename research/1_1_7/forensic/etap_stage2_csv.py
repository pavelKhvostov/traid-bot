"""Stage 2 CSV: SL sweep на winner entry_pct=0.8.

SL варианты:
  ob_full        — SL = ob.bottom (LONG) / ob.top (SHORT). Stage 1 default.
  ob_inside_15   — SL внутри OB на 15% от противоположной границы
  ob_inside_35   — SL внутри OB на 35%
  ob_inside_50   — SL внутри OB на 50%
  poi_extreme    — SL = poi.bottom (LONG) / poi.top (SHORT) = sweep.low/high

Все при entry_pct=0.8, RR=1.0.
"""
...
ENTRY_PCT = 0.8
RR = 1.0
SL_VARIANTS = ["ob_full", "ob_inside_15", "ob_inside_35", "ob_inside_50",
               "poi_extreme"]
...
def main():
    ...
    for _, t in df.iterrows():
        ...
        for sv in SL_VARIANTS:
            if d == "LONG":
                if sv == "ob_full":      sl = ob_b
                elif sv == "ob_inside_15": sl = ob_b + 0.15 * (ob_t - ob_b)
                elif sv == "ob_inside_35": sl = ob_b + 0.35 * (ob_t - ob_b)
                elif sv == "ob_inside_50": sl = ob_b + 0.50 * (ob_t - ob_b)
                elif sv == "poi_extreme":  sl = poi_b
                if sl >= entry:
                    rows.append(_inv_row(t, sv, entry, sl, d))
                    continue
                tp = entry + RR * (entry - sl)
            else:
                if sv == "ob_full":      sl = ob_t
                elif sv == "ob_inside_15": sl = ob_t - 0.15 * (ob_t - ob_b)
                elif sv == "ob_inside_35": sl = ob_t - 0.35 * (ob_t - ob_b)
                elif sv == "ob_inside_50": sl = ob_t - 0.50 * (ob_t - ob_b)
                elif sv == "poi_extreme":  sl = poi_t
                if sl <= entry:
                    rows.append(_inv_row(t, sv, entry, sl, d))
                    continue
    ...
    for sv in SL_VARIANTS:
        ...
        summary.append({
            ...
            "not_filled": (sub["outcome"] == "not_filled").sum(),
            "open": (sub["outcome"] == "open").sum(),
            "invalid": (sub["outcome"] == "invalid").sum(),
            "avg_risk_%": round(avg_risk, 2),
            "WR_%": round(wr, 1),
            "total_R": round(total, 1),
            "R_per_trade": round(r_tr, 3),
        })
    sm_df = pd.DataFrame(summary)
    sm_path = OUT_DIR / "stage2_sl_summary.csv"
    sm_df.to_csv(sm_path, index=False)
    print(f"  saved summary:   {sm_path}")
    print("\n" + sm_df.to_string(index=False))


def _inv_row(t, sv, entry, sl, d):
    return {
        "signal_time": t["signal_time_utc"].isoformat(),
        "direction": d, "sl_variant": sv,
        "entry": round(entry, 2), "sl": round(sl, 2), "tp": np.nan,
        "risk_pct": np.nan,
