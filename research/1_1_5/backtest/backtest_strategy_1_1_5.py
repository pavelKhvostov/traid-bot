"""Бэктест Strategy 1.1.5 на BTCUSDT — raw baseline.

Логика:
  - Сбор сигналов через strategies.strategy_1_1_5.detect_strategy_1_1_5_signals
  - Симуляция: limit-вход = top RDRB-зоны (LONG) / bottom RDRB-зоны (SHORT).
    Касание = entry. no_entry: если до touch цена коснулась TP или SL — отмена.
  - SL = min(c1.low, c2.low) (LONG) / max(c1.high, c2.high) (SHORT). RR=1.0.
...
Дедуп: двухэтапный bucketing (entry 0.5%, SL 0.5%) — как в 1.1.6.
"""
...
from strategies.strategy_1_1_5 import RR, detect_strategy_1_1_5_signals

DAYS_BACK = 1095
SYMBOL = "BTCUSDT"
OUTPUT = Path("signals/backtest_strategy_1_1_5.csv")
...
def _make_base_row(sig: dict) -> dict:
    ...
    return {
        ...
        "direction": sig["direction"],
        "top_tf": sig["top_tf"],
        "ob_top_cur_time": to_utc3(sig["ob_top_cur_time"]),
        "ob_top_zone": zone_str(sig["ob_top_zone"]),
        "macro_tf": sig["macro_tf"],
        "fvg_macro_c2_time": to_utc3(sig["fvg_macro_c2_time"]),
        "fvg_macro_zone": zone_str(sig["fvg_macro_zone"]),
        "htf_tf": sig["htf_tf"],
        "rdrb_c4_time": to_utc3(sig["rdrb_c4_time"]),
        "rdrb_zone": zone_str(sig["rdrb_zone"]),
        "entry": entry,
        "sl": sig["sl"],
        "tp": sig["tp"],
        "risk": risk,
...
def dedupe_signals(rows: list[dict]) -> list[dict]:
    """Двухэтапный bucketing (entry 0.5%, SL 0.5%). Аналог 1.1.6."""
    ...
    for group in final_buckets:
        ...
        top_times = [r["ob_top_cur_time"] for r in group]
        macro_times = [r["fvg_macro_c2_time"] for r in group]
        rdrb_times = [r["rdrb_c4_time"] for r in group]
        ...
        new = {
            ...
            "ob_top_cur_time": min(top_times),
            "ob_top_zone": first["ob_top_zone"],
            "macro_tf": ",".join(macro_tfs),
            "macro_tf_count": len(macro_tfs),
            "fvg_macro_c2_time": min(macro_times),
            "fvg_macro_zone": first["fvg_macro_zone"],
            "htf_tf": ",".join(htf_tfs),
            "htf_tf_count": len(htf_tfs),
            "rdrb_c4_time": min(rdrb_times),
            "rdrb_zone": first["rdrb_zone"],
            "entry": first["entry"],
            "sl": first["sl"],
            "tp": first["tp"],
            "risk": first["risk"],
...
def main():
    print(f"[INFO] Strategy 1.1.5 raw backtest, {SYMBOL}, окно {DAYS_BACK}d, RR={RR}\n")
    ...
    raw_signals = detect_strategy_1_1_5_signals(
    ...
    print()
    print("=" * 72)
    print(f"СВОДКА  Strategy 1.1.5  RR={RR}  окно={DAYS_BACK}d  {SYMBOL}")
    print("=" * 72)
    ...
    if not closed.empty:
        c = closed.copy()
        c["t"] = pd.to_datetime(c["rdrb_c4_time"])
        c["year"] = c["t"].dt.year
