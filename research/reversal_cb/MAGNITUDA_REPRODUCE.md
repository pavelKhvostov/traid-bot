...
## 8. Улучшение (вола-гейт) — в работе
Регим-тест → брать сигнал только при **низкой воле** (`atr_ptile ≤ порог`, известен на входе). Цель: вытянуть слабые
2023-24 (высоковола-бык) из минуса, поднять Sharpe, НЕ обнуляя сделки. Скрипт `vol_gate.py` (свип порога 1.0/0.7/0.5/0.3).
