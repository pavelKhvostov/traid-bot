"""Этап 61: 1.1.4 точно по user-спецификации.

Логика (от пользователя):

  L1: FVG-d/12h (макро якорь)
     - active с c2_close до invalidation OR life_end
     - invalidation: свеча того же ТФ с
        low < FVG.bottom для LONG
        high > FVG.top для SHORT

  L2: OB-4h/6h
     - может быть ВНУТРИ времени L1 ИЛИ ПОСЛЕ L1 c2_close
     - в пределах L1 active period (до invalidation)
     - same direction, overlap по цене с L1

  L3+L4 пара (после L2 cur_close, retest):

  L3: OB-1h/2h
     - search start = L2.cur_close
     - overlap по цене с L1 И L2

  L4: FVG-15m/20m
     - sync с L3 (как 1.1.1 — c0_time >= L3.prev_time, c2_close <= L3.cur_close)
     - overlap по цене С L1 И L2 (по требованию user)
"""
...
def find_invalidation(df_top, fvg_top, top_td, life_end):
    """Находит первую свечу на TF FVG-top где цена выходит за пределы FVG-зоны.

    Returns: timestamp invalidation или None если не найдено.
    """
    ...
    if fvg_top["direction"] == "LONG":
        # invalidation = свеча с low < FVG.bot
        mask = df_window["low"] < fvg_top["bottom"]
    else:
        mask = df_window["high"] > fvg_top["top"]
    if not mask.any(): return None
    first_inval = df_window.index[mask][0]
    return first_inval


def detect_114_v3(fvgs_top, obs_macro, obs_mid, fvgs_entry,
                   top_tf, macro_tf, mid_tf, entry_tf,
                   df_top):
    """1.1.4 по точной user-спецификации."""
    setups = []
    ...
    for fvg_top in fvgs_top:
        ...
        # L2: OB-4h/6h в окне [L1.c0_open, L1_active_end]
        # Может быть до L1.c2_close или после
        for ob_macro in obs_macro:
            ob_macro_close = ob_macro["time"] + macro_td
            # OB полностью inside L1 active period:
            #   prev_time >= L1.c0_open
            #   cur_close <= L1_active_end
            ...
            # L3+L4 пара: search start = L2.cur_close (retest)
            ...
            for oj in range(j0, j1):
                ...
                for ek in range(k0, len(fvgs_entry_sorted)):
                    ...
                    # Overlap по цене с L1 И L2 (по user-spec)
            ...
            setups.append({
                ...
                "signal_time": L3_close,  # detection at L3.cur_close
                "ob_htf_tf": mid_tf,
                "ob_htf_cur_time": ob_mid_found["time"],
                "ob_htf_prev_time": ob_mid_found["prev_time"],
            })
            break  # first L2-L3-L4 chain per FVG-top
    return setups
...
def main():
    ...
    print("[INFO] detect 1.1.4 v3 (user-spec)")
    ...
    for top_tf, top_zones in [("1d", fvgs_1d), ("12h", fvgs_12h)]:
        for macro_tf, macro_zones in [("4h", obs_4h), ("6h", obs_6h)]:
            for mid_tf, mid_zones in [("1h", obs_1h), ("2h", obs_2h)]:
                chains = detect_114_v3(top_zones, macro_zones, mid_zones,
    ...
    for c in candidates[:5]:
        print(f"    {c['label']:<35} n={c['n']:>3} WR={c['wr']:5.1f}% "
              f"total={c['total_R']:+6.1f}R  bad={c['bad_yrs']}/{c['n_yrs']}")
