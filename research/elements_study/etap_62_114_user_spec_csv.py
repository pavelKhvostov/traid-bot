"""Этап 62: CSV позиций для 1.1.4 user-spec + do_match.

Параметры:
  - Каскад user-spec (etap_61): L1 FVG-d + L2 OB-4h ВНУТРИ L1 active period
    с invalidation tracking + L3+L4 retest после L2.cur_close,
    sync L4 с L3, overlap L4 with L1 AND L2
  - SL anchor: x1 = пересечение FVG-1d ∩ OB-4h
  - entry=0.7, sl=0.35L/0.65S, min_sl=1%
  - RR=1.8 (clean baseline + do_match)
...
OUT_FULL = OUT_DIR / "etap62_114_userspec_positions_full.csv"
OUT_HUMAN = OUT_DIR / "etap62_114_userspec_positions_human.csv"
...
def main():
    ...
    print("[INFO] detect 1.1.4 user-spec")
    ...
    cols_human = [
        ("trade_id", "№"),
        ("anchor_c2_close_utc", "FVG-d/12h готов (c2 close)"),
        ("anchor_tf", "ТФ якоря"),
        ("L1_invalidated", "L1 invalidated?"),
        ("macro_close_utc", "OB-4h/6h cur close"),
        ("macro_tf", "ТФ макро OB"),
        ("mid_close_utc", "OB-1h/2h cur close (= signal)"),
        ("mid_tf", "ТФ mid OB"),
    ...
    df_human["swept"] = df_full["swept"].map({True: "✓ SWEPT", False: "не SWEPT"}).fillna("")
    ...
    print(f"  Найдено сетапов unique:               {len(unique)}")
    print(f"  Отрезано do_match counter:            {skipped_dom}")
    print(f"  Прошло do_match aligned:              {len(df_full)}")
    print(f"  L1 invalidated (FVG-1d убита):        {df_full['L1_invalidated'].sum()}")
