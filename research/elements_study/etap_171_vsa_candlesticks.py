"""etap_171: VSA (Williams) + Candlesticks (Nison) features.

База: etap_170 (Lopez + zone strength + USDT.D + sweep).
Добавлено ~30 фичей из VSA-методологии Williams и свечных паттернов Nison.

VSA cluster: no_demand, no_supply, stopping_volume, effort_no_result,
             absorption, test_bar, climax_bull/bear
Candle patterns: engulfing, hammer, hanging_man, shooting_star,
                 inverted_hammer, morning/evening_star, harami(+cross)
Doji family: is_doji, gravestone_at_top, dragonfly_at_bottom,
             doji_after_long_white, doji_rarity_30d
VSA context: phase_state, vol_up_down_ratio, climax_memory,
             smart_money_pressure
Confluence meta: confluence_score (0-10)

Methodology: Purged K-Fold CV + sample weights (наследовано от etap_170).
...
def build_dataset(df_12h, df_1d, df_4h, df_2h, df_1h, df_usdtd_1d=None):
    ...
    # === VSA + CANDLESTICK FEATURES (precompute, бар-уровень) ===
    print('  computing VSA + Candlestick features (Williams + Nison)...')
    _op = df_12h['open'].values
    _cl = df_12h['close'].values
    _hi = df_12h['high'].values
    _lo = df_12h['low'].values
    _vol = df_12h['volume'].values
    _rng = _hi - _lo
    _body = np.abs(_cl - _op)
    _body_pct = _body / np.where(_rng == 0, 1, _rng)
    _upper_wick = _hi - np.maximum(_op, _cl)
    _lower_wick = np.minimum(_op, _cl) - _lo
    _upper_wick_pct = _upper_wick / np.where(_rng == 0, 1, _rng)
    _lower_wick_pct = _lower_wick / np.where(_rng == 0, 1, _rng)
    _close_pos = (_cl - _lo) / np.where(_rng == 0, 1, _rng)
    _is_bull = _cl > _op

    _atr14 = atr(df_12h, 14).values
    _spread_atr = _rng / np.where(np.isnan(_atr14) | (_atr14 == 0), 1, _atr14)
    _mean_body_20 = pd.Series(_body).rolling(20).mean().values

    _sma50 = pd.Series(_cl).rolling(50).mean().values
    _sma_slope = (_sma50 - pd.Series(_sma50).shift(5).values) / np.where(np.isnan(_sma50), 1, _sma50)
    _uptrend = (_cl > _sma50) & (_sma_slope > 0)
    _downtrend = (_cl < _sma50) & (_sma_slope < 0)

    _vol_mean20 = pd.Series(_vol).rolling(20).mean().values
    _vol_std20 = pd.Series(_vol).rolling(20).std().values
    _vsa_vz = (_vol - _vol_mean20) / np.where(_vol_std20 == 0, 1, _vol_std20)

    n = len(df_12h)

    # --- VSA cluster ---
    no_demand = (_is_bull & (_spread_atr < 0.5) & (_vsa_vz < -0.5) & _uptrend).astype(int)
    no_supply = ((~_is_bull) & (_spread_atr < 0.5) & (_vsa_vz < -0.5) & _downtrend).astype(int)
    stopping_vol = ((~_is_bull) & (_vsa_vz > 2.0) & (_spread_atr > 1.5) & (_close_pos > 0.66)).astype(int)
    absorption = ((~_is_bull) & (_vsa_vz > 2.0) & (_spread_atr > 1.5) & (_close_pos > 0.80)).astype(int)
    effort_no_result = np.zeros(n, dtype=int)
    for i in range(2, n):
        if (_vsa_vz[i-1] > 1.5) and (_spread_atr[i-1] > 1.3):
            move = abs(_cl[i] - _cl[i-2]) / max(_atr14[i-1], 1)
            if move < 0.3:
                effort_no_result[i] = 1

    recent_climax = pd.Series(stopping_vol | absorption).rolling(2).max().fillna(0).astype(bool).values
    test_bar = ((~_is_bull) & (_vsa_vz < -0.7) & (_close_pos > 0.6) & recent_climax).astype(int)

    _rng_mean20 = pd.Series(_rng).rolling(20).mean().values
    _rng_std20 = pd.Series(_rng).rolling(20).std().values
    z_range = (_rng - _rng_mean20) / np.where(_rng_std20 == 0, 1, _rng_std20)
    climax_bull = (np.clip(z_range, 0, None) * np.clip(_vsa_vz, 0, None) * _close_pos)
    climax_bear = (np.clip(z_range, 0, None) * np.clip(_vsa_vz, 0, None) * (1 - _close_pos))

    # --- Candlestick patterns ---
    hammer = ((_lower_wick_pct >= 0.5) & (_upper_wick_pct < 0.15) & (_body_pct < 0.4) & _downtrend).astype(int)
    hanging_man = ((_lower_wick_pct >= 0.5) & (_upper_wick_pct < 0.15) & (_body_pct < 0.4) & _uptrend).astype(int)
    shooting_star = ((_upper_wick_pct >= 0.5) & (_lower_wick_pct < 0.15) & (_body_pct < 0.4) & _uptrend).astype(int)
    inv_hammer = ((_upper_wick_pct >= 0.5) & (_lower_wick_pct < 0.15) & (_body_pct < 0.4) & _downtrend).astype(int)

    engulfing_bull = np.zeros(n, dtype=int)
    engulfing_bear = np.zeros(n, dtype=int)
    engulfing_ratio = np.zeros(n)
    for i in range(1, n):
        if _cl[i-1] < _op[i-1] and _cl[i] > _op[i]:
            if _op[i] <= _cl[i-1] and _cl[i] >= _op[i-1]:
                if _downtrend[i]:
                    engulfing_bull[i] = 1
                    engulfing_ratio[i] = _body[i] / max(_body[i-1], 1)
        elif _cl[i-1] > _op[i-1] and _cl[i] < _op[i]:
            if _op[i] >= _cl[i-1] and _cl[i] <= _op[i-1]:
                if _uptrend[i]:
                    engulfing_bear[i] = 1
                    engulfing_ratio[i] = -_body[i] / max(_body[i-1], 1)

    morning_star = np.zeros(n, dtype=int)
    evening_star = np.zeros(n, dtype=int)
    for i in range(2, n):
        b2 = _body[i-2]; b1 = _body[i-1]; b0 = _body[i]
        if (_cl[i-2] < _op[i-2]) and (b2 > 1.5 * max(_mean_body_20[i-2], 1)):
            if b1 < 0.3 * b2:
                if (_cl[i] > _op[i]) and (_cl[i] > (_op[i-2] + _cl[i-2]) / 2):
                    morning_star[i] = 1
        if (_cl[i-2] > _op[i-2]) and (b2 > 1.5 * max(_mean_body_20[i-2], 1)):
            if b1 < 0.3 * b2:
                if (_cl[i] < _op[i]) and (_cl[i] < (_op[i-2] + _cl[i-2]) / 2):
    ...
    harami = np.zeros(n, dtype=int)
    harami_cross = np.zeros(n, dtype=int)
    for i in range(1, n):
        if _body[i-1] > 1.5 * max(_mean_body_20[i-1], 1):
            top1 = max(_op[i-1], _cl[i-1]); bot1 = min(_op[i-1], _cl[i-1])
            top0 = max(_op[i], _cl[i]); bot0 = min(_op[i], _cl[i])
            if top0 < top1 and bot0 > bot1:
                harami[i] = 1
                if _body_pct[i] < 0.1:
                    harami_cross[i] = 1

    cloud_penetration = np.zeros(n)
    for i in range(1, n):
        if _cl[i-1] > _op[i-1] and _op[i] > _hi[i-1] and _cl[i] < _op[i]:
            mid = (_op[i-1] + _cl[i-1]) / 2
            cloud_penetration[i] = -(mid - _cl[i]) / max(_body[i-1], 1)
        elif _cl[i-1] < _op[i-1] and _op[i] < _lo[i-1] and _cl[i] > _op[i]:
            mid = (_op[i-1] + _cl[i-1]) / 2
            cloud_penetration[i] = (_cl[i] - mid) / max(_body[i-1], 1)

    # --- Doji family ---
    is_doji = ((np.abs(_cl - _op) / np.where(_atr14 == 0, 1, _atr14) < 0.05) & (_body_pct < 0.10)).astype(int)
    hh_30d = pd.Series(_hi).rolling(60).max().values
    ll_30d = pd.Series(_lo).rolling(60).min().values
    near_hh = (_hi / np.where(hh_30d == 0, 1, hh_30d) > 0.98).astype(int)
    near_ll = (_lo / np.where(ll_30d == 0, 1, ll_30d) < 1.02).astype(int)
    gravestone_at_top = (is_doji & (_upper_wick_pct > 0.7) & near_hh).astype(int)
    dragonfly_at_bottom = (is_doji & (_lower_wick_pct > 0.7) & near_ll).astype(int)
    long_legged_doji = (is_doji & (_upper_wick_pct > 0.35) & (_lower_wick_pct > 0.35)).astype(int)
    doji_after_long_white = np.zeros(n, dtype=int)
    for i in range(1, n):
        if is_doji[i] and (_body[i-1] > 1.5 * max(_atr14[i-1], 1)) and (_cl[i-1] > _op[i-1]):
            doji_after_long_white[i] = 1
    doji_count_30d = pd.Series(is_doji).rolling(60).sum().values
    doji_rarity = 1 / (1 + doji_count_30d)

    # --- VSA context ---
    vol_up = np.where(_is_bull, _vol, 0)
    vol_down = np.where(~_is_bull, _vol, 0)
    vol_up_60 = pd.Series(vol_up).rolling(60).mean().values
    vol_down_60 = pd.Series(vol_down).rolling(60).mean().values
    vol_ud_ratio = vol_up_60 / np.where(vol_down_60 == 0, 1, vol_down_60)

    any_climax = (stopping_vol | absorption | (climax_bear > 1).astype(int) | (climax_bull > 1).astype(int))
    climax_memory_60 = pd.Series(any_climax).rolling(60).max().fillna(0).astype(int).values

    narrow_bar = (_spread_atr < 0.7).astype(int)
    smart_money_raw = _vol * np.sign(_close_pos - 0.5) * (2 * narrow_bar - 1)
    smart_money = pd.Series(smart_money_raw).rolling(20).sum().fillna(0).values

    phase_accum = ((_cl < _sma50) & (vol_ud_ratio < 1) & climax_memory_60.astype(bool)).astype(int)
    phase_markup = ((_cl > _sma50) & (_sma_slope > 0)).astype(int)
    phase_distrib = ((_cl > _sma50) & (vol_ud_ratio < 1) & near_hh.astype(bool)).astype(int)
    phase_markdown = ((_cl < _sma50) & (_sma_slope < 0)).astype(int)

    print('  VSA + Candlestick features done')
    ...
    for i in range(2, len(df_12h)):
        ...
        # === NEW: VSA + Candlestick features ===
        # VSA cluster
        feat['vsa_no_demand'] = int(no_demand[i])
        feat['vsa_no_supply'] = int(no_supply[i])
        feat['vsa_stopping_vol'] = int(stopping_vol[i])
        feat['vsa_absorption'] = int(absorption[i])
        feat['vsa_effort_no_result'] = int(effort_no_result[i])
        feat['vsa_test_bar'] = int(test_bar[i])
        feat['vsa_climax_bull'] = float(climax_bull[i]) if not np.isnan(climax_bull[i]) else 0.0
        feat['vsa_climax_bear'] = float(climax_bear[i]) if not np.isnan(climax_bear[i]) else 0.0
        # Candle patterns
        feat['cdl_hammer'] = int(hammer[i])
        feat['cdl_hanging_man'] = int(hanging_man[i])
        feat['cdl_shooting_star'] = int(shooting_star[i])
        feat['cdl_inv_hammer'] = int(inv_hammer[i])
        feat['cdl_engulfing_bull'] = int(engulfing_bull[i])
        feat['cdl_engulfing_bear'] = int(engulfing_bear[i])
        feat['cdl_engulfing_ratio'] = float(engulfing_ratio[i])
        feat['cdl_morning_star'] = int(morning_star[i])
        feat['cdl_evening_star'] = int(evening_star[i])
        feat['cdl_harami'] = int(harami[i])
        feat['cdl_harami_cross'] = int(harami_cross[i])
        feat['cdl_cloud_penetration'] = float(cloud_penetration[i])
        # Doji family
        feat['doji_is'] = int(is_doji[i])
        feat['doji_gravestone_at_top'] = int(gravestone_at_top[i])
        feat['doji_dragonfly_at_bottom'] = int(dragonfly_at_bottom[i])
        feat['doji_long_legged'] = int(long_legged_doji[i])
        feat['doji_after_long_white'] = int(doji_after_long_white[i])
        feat['doji_rarity_30d'] = float(doji_rarity[i]) if not np.isnan(doji_rarity[i]) else 1.0
        # VSA context
        feat['vsa_vol_up_down_ratio_60'] = float(vol_ud_ratio[i]) if not np.isnan(vol_ud_ratio[i]) else 1.0
        feat['vsa_climax_memory_60'] = int(climax_memory_60[i])
        feat['vsa_smart_money_20'] = float(smart_money[i])
        feat['vsa_phase_accum'] = int(phase_accum[i])
        feat['vsa_phase_markup'] = int(phase_markup[i])
        feat['vsa_phase_distrib'] = int(phase_distrib[i])
        feat['vsa_phase_markdown'] = int(phase_markdown[i])

        # === Confluence meta (0-10) — Nison style ===
        conf = 0
        # +2 reversal candle
        if hammer[i] or inv_hammer[i] or engulfing_bull[i] or morning_star[i] or dragonfly_at_bottom[i]: conf += 2
        if hanging_man[i] or shooting_star[i] or engulfing_bear[i] or evening_star[i] or gravestone_at_top[i]: conf += 2
        # +2 рядом с SMA65 (используем ema_200 как proxy)
        if abs(feat.get('ema_200_dist_pct', 0)) < 1.0: conf += 2
        # +1 rsi extreme
        if feat.get('rsi_14', 50) > 70 or feat.get('rsi_14', 50) < 30: conf += 1
        # +1 volume confirmation
        if feat.get('vol_zscore_20', 0) > 1.0: conf += 1
        # +1 sweep failed
        if feat.get('sweep_BSL_failed_24h', 0) or feat.get('sweep_SSL_failed_24h', 0): conf += 1
        # +1 при наличии stopping_vol или absorption (VSA)
        if stopping_vol[i] or absorption[i]: conf += 1
        feat['confluence_score'] = min(conf, 10)
...
def main():
    print("="*70)
    print("etap_171: VSA (Williams) + Candlesticks (Nison) features")
    print("  + Lopez methodology (Purged K-Fold + sample weights)")
    print("="*70)
    print()
    ...
    for direction in ['low', 'high']:
        for t in TARGETS:
            ...
            pdf['p_hit'] = p_test
            pdf.to_csv(out_dir / f'etap_171_pred_{col}.csv', index=False)
    ...
    sdf.to_csv(out_dir / 'etap_171_summary.csv', index=False)
    print("="*70)
    print("SUMMARY")
    print("="*70)
