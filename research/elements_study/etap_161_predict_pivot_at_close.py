"""etap_161: Predict pivot AT close — без 24ч lookahead.

ОТЛИЧИЕ от etap_160:
  etap_160: фрактал УЖЕ подтверждён (через 24ч после экстремума) →
            предсказываем хватит ли движения. Lag 24ч.
  etap_161: на close ЛЮБОЙ 12h свечи → предсказываем
            "будет ли эта свеча признана LOW/HIGH фракталом + даст ли
            движение ≥X% от её low/high в течение 14 баров после
            confirmation". Lag = 0 (предсказываем в момент close).
...
def build_dataset(df_12h, df_1d, df_4h, df_2h, df_1h):
    ...
    for i in range(2, len(df_12h)):
        ...
        # HTF trend (1d/4h Hull direction)
        try:
            h1d_at = hull_1d.asof(ts_i); h1d_prev = hull_1d.asof(ts_i - pd.Timedelta(days=3))
            feat['htf_1d_hull_dir'] = 1 if h1d_at > h1d_prev else -1
        except Exception:
            feat['htf_1d_hull_dir'] = 0
        try:
            h4h_at = hull_4h.asof(ts_i); h4h_prev = hull_4h.asof(ts_i - pd.Timedelta(hours=12))
            feat['htf_4h_hull_dir'] = 1 if h4h_at > h4h_prev else -1
        except Exception:
        ...
        # ЗОНЫ 1d/4h/2h/1h через bisect
        def _zones_for(times, objs, lb_bars, bar_min):
            cut = bisect.bisect_right(times, ts_i)
            cut_lo = bisect.bisect_left(times, ts_i - pd.Timedelta(minutes=bar_min*lb_bars))
            return objs[cut_lo:cut]
...
def main():
    print("="*70)
    print("etap_161: Predict pivot AT CLOSE — без 24ч lookahead")
    print("="*70)
    print()
    ...
    for direction in ['low', 'high']:
        for t in TARGETS:
            ...
            pdf['p_hit'] = p_test
            pdf.to_csv(out_dir / f'etap_161_pred_{col}.csv', index=False)
    ...
    sdf.to_csv(out_dir / 'etap_161_summary.csv', index=False)
    print("="*70)
    print("SUMMARY")
    print("="*70)
