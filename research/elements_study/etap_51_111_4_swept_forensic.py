"""Этап 51: 1.1.4 + SWEPT + полный indicator forensic.

Подход аналогичен etap_47 (для 1.1.2):
  Phase A: SAFE baseline 1.1.4 + SWEPT filter
...
ВАЖНО:
  - SAFE simulator (no_entry + not_filled checks из etap_46)
  - Lookahead-safe lookup для всех индикаторов (last closed bar)
  - Cyrillic в print заменён на латиницу для cp1251 console
"""
    ...
    for fvg_top in fvgs_top:
        ...
        for mi in range(i0, i1):
            ...
            setups.append({
                ...
                "ob_mid": ob_mid_found,
                "ob_macro": ob_macro,
                "fvg_top_bot": fvg_top["bottom"], "fvg_top_top": fvg_top["top"],
...
def check_swept(sig, df_1h, df_2h):
    """SWEPT для OB-mid (L3): 2 свечи OB pair выносят low/high 2 предыдущих свечей."""
...
def report(label, df):
    ...
    if cl.empty:
        print(f"  {label}: no closed (n_total={n_total})"); return None
    nc = len(cl)
    wr = (cl["R"] > 0).mean() * 100
    tot = cl["R"].sum()
    rt = cl["R"].mean()
    ...
    return {"closed": nc, "wr": wr, "total_R": tot, "rt": rt,
...
def evaluate_filter(closed_df, mask_label, mask, baseline_wr, baseline_R):
    ...
    print(f"    WR={wr:.1f}% (d={wr-baseline_wr:+.1f}pp) total={tot:+.1f}R "
          f"R/tr={rt:+.3f} bad_yrs={bad}/{len(yr)}")
...
def main():
    ...
    print("[INFO] compute indicators (Hull lengths, EMA200, ASVK, MH)")
    ...
    print("[INFO] detect 1.1.4 chains (8 combos)")
    all_setups = []
    for top_tf, top_zones in [("1d", fvgs_1d), ("12h", fvgs_12h)]:
        for macro_tf, macro_zones in [("4h", obs_4h), ("6h", obs_6h)]:
            for mid_tf, mid_zones in [("1h", obs_1h), ("2h", obs_2h)]:
                chains = detect_114_chain(top_zones, macro_zones, mid_zones,
    ...
    for s in unique:
        ...
        if sw:
            swept_setups.append(s)
    print(f"  SWEPT-filtered (cur+prev OB-mid taken out previous 2 candle low/high): {len(swept_setups)}")

    # ============================================================
    # PHASE A: BASELINES
    # ============================================================
    print(f"\n{'='*70}\nPHASE A: BASELINES (USER params, RR=1.8)")
    print(f"{'='*70}")
    print("\n  ALL signals (no SWEPT):")
    df_all = evaluate(unique, RR, df_1m)
    base_all = report("ALL", df_all)

    print("\n  SWEPT-filtered signals only:")
    df_swept = evaluate(swept_setups, RR, df_1m)
    base_swept = report("SWEPT", df_swept)

    # also try other RR on SWEPT
    print("\n  SWEPT cache, RR sweep:")
    for rr in [1.0, 1.5, 1.8, 2.0, 2.5]:
        df_e = evaluate(swept_setups, rr, df_1m)
        report(f"SWEPT RR={rr}", df_e)

    # Choose better baseline for forensic
    use_swept = base_swept and base_swept["total_R"] > (base_all["total_R"] if base_all else -999)
    if use_swept:
        cache = swept_setups
        df_base = df_swept
        base_label = "SWEPT"
        print(f"\n  -> USE SWEPT cache for forensic")
    else:
        cache = unique
        df_base = df_all
        base_label = "ALL"
        print(f"\n  -> USE ALL cache for forensic")

    closed = df_base[df_base["outcome"].isin(["win", "loss"])].copy()
    base_wr = (closed["R"] > 0).mean() * 100 if len(closed) else 0
    base_R = closed["R"].sum()
    print(f"\n  BASELINE for forensic: {base_label} RR={RR}  "
          f"WR={base_wr:.1f}%  total={base_R:+.1f}R  n={len(closed)}")

    # ============================================================
    # PHASE B: feature extraction
    # ============================================================
    print(f"\n[PHASE B] extract features for {len(closed)} closed trades")
    feat_rows = []
    ...
    csv_out = OUT_DIR / f"etap51_114_{base_label}_features.csv"
    ...
    # ============================================================
    print(f"\n{'='*70}\nPHASE C: PER-FEATURE SEGMENTATION  "
          f"(baseline {base_wr:.1f}% / {base_R:+.1f}R)")
    print(f"{'='*70}")
    ...
    # Hull sweep (16 variants)
    print(f"\n{'='*70}\nHULL parameter sweep (TF x Length)")
    print(f"{'='*70}")
    for hull_key in sorted(hull_lbls.keys()):
        col = f"hull_{hull_key}_align"
        report_segment(closed, col, base_wr, base_R, min_n=20)
    ...
    for r in sorted_r[:10]:
        ...
        print(f"  {r['name']:<32} n={r['n']:>4} WR={r['wr']:5.1f}% "
              f"total={r['total_R']:+6.1f}R R/tr={r['rt']:+.3f}{flag}")
    ...
    combo_filters = [
        ...
        ("LONG + OB medium",
            (closed["direction"] == "LONG") &
            (closed["ob_depth_bin"] == "medium")),
        ("LONG + ema200_1h aligned",
            (closed["direction"] == "LONG") &
            (closed["ema200_1h_align"] == "aligned")),
    ]
    combo_results = []
    ...
    print(f"\n--- ALL combos sorted by total_R ---")
    all_sorted = sorted(combo_results, key=lambda x: x["total_R"], reverse=True)
    for r in all_sorted[:5]:
        flag = "" if r["bad_yrs"] == 0 else f" (bad={r['bad_yrs']})"
        print(f"  {r['name']:<35} n={r['n']:>4} WR={r['wr']:5.1f}% "
              f"total={r['total_R']:+6.1f}R{flag}")

    # ============================================================
    # FINAL VERDICT
    # ============================================================
    print(f"\n{'='*70}\nFINAL VERDICT for 1.1.4")
    print(f"{'='*70}")
    print(f"\n  Baseline ({base_label}): {base_R:+.1f}R / WR={base_wr:.1f}% / n={len(closed)}")
    all_results = results + combo_results
    winners = [r for r in all_results
                if r["total_R"] > base_R and r["bad_yrs"] == 0]
    winners = sorted(winners, key=lambda x: x["total_R"], reverse=True)
    print(f"\n  Variants beating baseline AND with 0 bad years:")
    if winners:
        for r in winners[:5]:
            print(f"    {r['name']:<35} n={r['n']:>4} total={r['total_R']:+.1f}R "
                  f"(d={r['total_R']-base_R:+.1f})")
    else:
        print(f"    NONE — baseline beats all filtered variants by total R")
