"""Этап 52: 1.1.4 STRICT-NESTED — все 4 уровня должны overlap'ить.

Различие vs etap_50/51:
  Старая (canonical 1.1.1-style): zones_overlap проверяется только
    между соседними уровнями (L2-L1, L3-L1+L2, L4-L3).

  STRICT-NESTED:
    - L4 FVG-15m ДОЛЖЕН overlap с L1 FVG-top (макро) ★
    - L4 FVG-15m ДОЛЖЕН overlap с L2 OB-macro ★
    - + всё что было раньше

Гипотеза: строгий nested = меньше «мусорных» setups, больше edge per trade.

Параметры USER (entry=0.7, sl=0.35/0.65, min_sl=1%, RR=1.8).
"""
...
def detect_114_strict(fvgs_top, obs_macro, obs_mid, fvgs_entry,
                       ...
                       require_l4_in_l1=True, require_l4_in_l2=True):
    """STRICT-NESTED версия: L4 FVG обязан overlap с L1 (FVG-top) и L2 (OB-macro)."""
    setups = []
    ...
    for fvg_top in fvgs_top:
        ...
        for mi in range(i0, i1):
            ...
            for ek in range(k0, k1):
                ...
                # L4 overlap с L3 (всегда)
                if not zones_overlap(f_entry["bottom"], f_entry["top"],
                                      ob_mid_found["bottom"], ob_mid_found["top"]):
                    continue
                # STRICT: L4 overlap с L1 (FVG-top)
                if require_l4_in_l1:
                    if not zones_overlap(f_entry["bottom"], f_entry["top"],
                                          fvg_top["bottom"], fvg_top["top"]):
                        continue
                # STRICT: L4 overlap с L2 (OB-macro)
                if require_l4_in_l2:
                    if not zones_overlap(f_entry["bottom"], f_entry["top"],
                                          ob_macro["bottom"], ob_macro["top"]):
                        continue
...
def main():
    ...
    # Three variants
    variants = [
        ("LOOSE  (canonical, etap_50)", False, False),
        ("STRICT-L1 only (L4 in FVG-top)", True, False),
        ("STRICT-L1+L2 (L4 in FVG-top AND OB-macro)", True, True),
    ]

    print(f"\n{'='*80}\nCOMPARISON: 3 variants of nested overlap")
    print(f"{'='*80}")

    for label, req_l1, req_l2 in variants:
        print(f"\n{label}:")
        all_setups = []
            ...
            for macro_tf, macro_zones in [("4h", obs_4h), ("6h", obs_6h)]:
                for mid_tf, mid_zones in [("1h", obs_1h), ("2h", obs_2h)]:
                    chains = detect_114_strict(top_zones, macro_zones, mid_zones,
                                                ...
                                                require_l4_in_l1=req_l1,
                                                require_l4_in_l2=req_l2)
                    all_setups.extend(chains)
        seen = set(); unique = []
        for s in all_setups:
            key = (s["signal_time"], s["direction"],
                    round(s["fvg_b"], 2), round(s["fvg_t"], 2))
