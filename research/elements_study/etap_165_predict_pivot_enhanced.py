"""etap_165: ENHANCED honest pivot model — добавлены USDT.D, sweep history,
multi-TF confluence, block_orders, iFVG, time-of-day, HTF range.

Базируется на etap_163 (HONEST, без lookahead). Добавлены новые фичи для
улучшения recall на "тихих топах" (5 мар, 6 мая, 10 мая, 14 мая 2026).
...
def main():
    print("="*70)
    print("etap_165: ENHANCED honest pivot — USDT.D + sweep + confluence + blocks + time")
    print("="*70)
    print()
    ...
    for direction in ['low', 'high']:
        for t in TARGETS:
            ...
            pdf['p_hit'] = p_test
            pdf.to_csv(out_dir / f'etap_165_pred_{col}.csv', index=False)
    ...
    sdf.to_csv(out_dir / 'etap_165_summary.csv', index=False)
    print("="*70)
    print("SUMMARY")
    print("="*70)
