"""etap_167: Zone-strength model.

База: etap_165. Добавлены фичи СИЛЫ зон (анализ TP vs FP показал что
у TP сильнее зональный контекст). Для каждой ближайшей same-dir и
opposite-dir зоны на каждом TF добавлены:
  - age_h (часы с рождения)
  - width_pct (узкая = сильнее)
  - birth_body_pct (body% свечи формировавшей зону)
  - touch_count (сколько касаний с рождения)
  - is_HTF_anchor (1 если 1d или 12h)
...
def main():
    print("="*70)
    print("etap_167: ZONE-STRENGTH model — age/width/touch/birth_momentum for nearest zones")
    print("="*70)
    print()
    ...
    for direction in ['low', 'high']:
        for t in TARGETS:
            ...
            pdf['p_hit'] = p_test
            pdf.to_csv(out_dir / f'etap_167_pred_{col}.csv', index=False)
    ...
    sdf.to_csv(out_dir / 'etap_167_summary.csv', index=False)
    print("="*70)
    print("SUMMARY")
    print("="*70)
