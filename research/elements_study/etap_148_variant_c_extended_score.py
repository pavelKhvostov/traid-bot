"""etap_148: Variant C extended score threshold sweep.

etap_146 tested score>=0 / 0.25 / 0.50. Расширяем:
  threshold ∈ {-0.50, -0.25, 0, +0.25, +0.50, +0.75, +1.00}

Поиск:
  - sweet spot per WR/PnL tradeoff
  - возможна ли premium-Tier выше +0.50 (например +0.75, +1.00)?
  - помогает ли +0.50/-0.25 (более лояльный) больше чем +0.50?
"""
...
THRESHOLDS = [-0.50, -0.25, 0.0, 0.25, 0.50, 0.75, 1.00]


def collect_with_scores(symbol, start_date):
...
def evaluate_threshold(setups, df_1m, df_1h, sl, ss, cap, th, cf, threshold):
    ...
    return {"n": n, "wr": W/n*100, "pnl": pnl, "rpt": pnl/n,
            "bad": bad, "ytot": len(yr_map), "dd": dd, "n_setups": len(filtered)}


def main():
    print("etap_148: Variant C extended score threshold sweep")
    print()
    per_sym = {}
    for sym, start, cap, th, cf in SYMBOLS:
        print(f"  Computing {sym}...")
        setups, df_1m, df_1h, sl, ss = collect_with_scores(sym, start)
    ...
    for sym in per_sym:
        ...
        print(f"\n  {sym}:")
        print(f"    {'threshold':>10}  {'setups':>6}  {'n':>3}  {'WR':>5}  {'PnL':>8}  {'R/tr':>6}  {'bad':>5}  {'DD':>7}")
        for t_score in THRESHOLDS:
            m = evaluate_threshold(setups, df_1m, df_1h, sl, ss, cap, th, cf, t_score)
            if m is None:
                print(f"    {t_score:>+10.2f}  no data"); continue
            mark = "  <- tier-1 etap_146" if abs(t_score - 0.50) < 1e-3 else ""
            print(f"    {t_score:>+10.2f}  {m['n_setups']:>6d}  {m['n']:>3d}  {m['wr']:>4.1f}%  "
                  f"{m['pnl']:>+6.1f}R  {m['rpt']:>+5.2f}  {m['bad']}/{m['ytot']}  -{m['dd']:>5.1f}R{mark}")

    # Portfolio per threshold
    print()
    print("  Portfolio BTC+ETH per threshold:")
    print("  " + "="*100)
    print(f"    {'threshold':>10}  {'n':>3}  {'WR':>5}  {'PnL':>8}  {'R/tr':>6}  {'bad_total':>9}  {'DD':>7}")
    for t_score in THRESHOLDS:
        all_trades = []
        for sym in per_sym:
            setups, df_1m, df_1h, sl, ss, cap, th, cf = per_sym[sym]
            filtered = [s for s in setups if not pd.isna(s["score"]) and s["score"] >= t_score]
            for s in filtered:
                outc, R = simulate_floating(s, s["entry"], s["sl"], df_1m, df_1h,
                                             sl, ss, R_cap=cap, threshold=th, confirm=cf)
                if outc in ("win", "loss"):
        ...
        for t in all_trades:
            cum += t["R"]; peak = max(peak, cum); dd = max(dd, peak - cum)
        mark = "  <- Variant D" if abs(t_score - 0.50) < 1e-3 else ""
        print(f"    {t_score:>+10.2f}  {n:>3d}  {W/n*100:>4.1f}%  {pnl:>+6.1f}R  {pnl/n:>+5.2f}  "
              f"{bad:>3d}/{len(yr_map)}  -{dd:>5.1f}R{mark}")
