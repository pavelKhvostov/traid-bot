"""Этап 64: 1.1.4 — сравнение 3 интерпретаций «OB в зоне FVG».

Какие зоны:
  LONG OB:  [min(prev.low, cur.low), prev.open]   ← prev.open = top = ORIGIN
  SHORT OB: [prev.open, max(prev.high, cur.high)] ← prev.open = bottom = ORIGIN
  LONG FVG:  [c0.high, c2.low]    (узкий gap)
  SHORT FVG: [c2.high, c0.low]

Интерпретации «OB-4h в зоне FVG-1d»:
  V_OVERLAP:    хоть в одной точке (etap_61 — было 231 setups)
  V_INCLUSION:  OB.bot >= FVG.bot AND OB.top <= FVG.top (etap_63 — было 47)
  V_ORIGIN:     ORIGIN OB inside FVG range (LONG: prev.open ∈ FVG; SHORT: prev.open ∈ FVG)
                ← самая семантически осмысленная
  V_ANY_EDGE:   OB.top OR OB.bottom inside FVG
"""
...
def collect_obs(df, atr, tf):
    """OB с дополнительным полем 'origin' = prev.open."""
    out = []
...
def zone_inside_full(small_b, small_t, big_b, big_t):
    return small_b >= big_b and small_t <= big_t


def origin_inside(origin, big_b, big_t):
    return big_b <= origin <= big_t


def any_edge_inside(ob_b, ob_t, big_b, big_t):
    """Хотя бы одна грань OB внутри FVG."""
...
def detect_114(fvgs_top, obs_macro, obs_mid, fvgs_entry,
                top_tf, macro_tf, mid_tf, entry_tf, df_top,
                inclusion_mode):
    """inclusion_mode: 'overlap' | 'inclusion' | 'origin' | 'any_edge'"""
    setups = []
    ...
    def ob_in_fvg(ob, fvg):
        if inclusion_mode == "overlap":
            return zones_overlap(ob["bottom"], ob["top"], fvg["bottom"], fvg["top"])
        if inclusion_mode == "inclusion":
            return zone_inside_full(ob["bottom"], ob["top"], fvg["bottom"], fvg["top"])
        if inclusion_mode == "origin":
            return origin_inside(ob["origin"], fvg["bottom"], fvg["top"])
        if inclusion_mode == "any_edge":
            return any_edge_inside(ob["bottom"], ob["top"], fvg["bottom"], fvg["top"])
        return False

    def ob_in_ob(ob, big_ob):
        # для L3 in L2 — те же правила
        if inclusion_mode == "overlap":
            return zones_overlap(ob["bottom"], ob["top"], big_ob["bottom"], big_ob["top"])
        if inclusion_mode == "inclusion":
            return zone_inside_full(ob["bottom"], ob["top"], big_ob["bottom"], big_ob["top"])
        if inclusion_mode == "origin":
            return origin_inside(ob["origin"], big_ob["bottom"], big_ob["top"])
        if inclusion_mode == "any_edge":
            return any_edge_inside(ob["bottom"], ob["top"], big_ob["bottom"], big_ob["top"])
        return False
    ...
    for fvg_top in fvgs_top:
        ...
        for ob_macro in obs_macro:
            ...
            if not ob_in_fvg(ob_macro, fvg_top): continue
            ...
            for oj in range(j0, j1):
                ...
                if not ob_in_fvg(ob_mid, fvg_top): continue
                if not ob_in_ob(ob_mid, ob_macro): continue
                ...
                for ek in range(k0, len(fvgs_entry_sorted)):
                    ...
                    # L4 — overlap (как user сказал "хотя бы одной частью")
...
def report_oneline(label, df):
    cl = df[df["outcome"].isin(["win", "loss"])]
    if cl.empty:
        return f"  {label}: no closed"
    nc = len(cl); wins = (cl["R"] > 0).sum()
    wr = wins/nc*100; tot = cl["R"].sum()
    yr = cl.groupby("year")["R"].sum()
    bad = (yr < 0).sum()
    return f"  {label}: n={nc:>3} WR={wr:5.1f}% total={tot:+6.1f}R bad={bad}/{len(yr)}"
...
def main():
    ...
    for mode in ["overlap", "inclusion", "origin", "any_edge"]:
        print(f"\n{'='*70}\n=== inclusion_mode = {mode} ===")
        print(f"{'='*70}")
        all_setups = []
        for top_tf, top_zones in [("1d", fvgs_1d), ("12h", fvgs_12h)]:
            for macro_tf, macro_zones in [("4h", obs_4h), ("6h", obs_6h)]:
                for mid_tf, mid_zones in [("1h", obs_1h), ("2h", obs_2h)]:
                    chains = detect_114(top_zones, macro_zones, mid_zones,
                                         fvgs_15m, top_tf, macro_tf, mid_tf, "15m",
                                         df_top_map[top_tf], mode)
        ...
        for rr in [1.5, 1.8, 2.0, 2.5, 3.0]:
            df_e = evaluate(unique, rr, df_1m, df_1d, only_dom=False)
            print(report_oneline(f"    RR={rr}", df_e))
