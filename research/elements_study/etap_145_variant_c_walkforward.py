"""etap_145: Walk-forward validation Variant C (V2 + A2 combined portfolio).

Variant C: A2 + (V2\A2) combined trades с Variant-B-style per-symbol floating.
В sample (etap_144): 513 trades / WR 52.6% / +129R / 1 bad (2020 -0.5R flat) / DD -8.4R.

Если Variant C удержит positive PnL в OOS test years, можно использовать как
primary candidate. Если OOS PnL обвалится -- Variant B остаётся primary.
...
def main():
    print("etap_145: Walk-forward Variant C (V2+A2 combined, per-symbol floating)")
    print()
    per_sym = {}
    for sym, start, cap, th, cf in SYMBOLS:
        print(f"  Computing {sym}...")
        per_sym[sym] = collect_variant_c_trades(sym, start, cap, th, cf)
        print(f"    {sym}: {len(per_sym[sym])} closed trades")

    # Merge
    all_trades = []
    for sym in per_sym:
        for t in per_sym[sym]:
            t2 = dict(t); t2["symbol"] = sym
            all_trades.append(t2)
    all_trades.sort(key=lambda t: t["time"])
    print(f"  Total combined trades: {len(all_trades)}")

    print()
    print("  Year-by-year Variant C portfolio:")
    print("  " + "="*80)
    ...
    print()
    print("  Walk-forward (cumulative train ↔ single test year):")
    print("  " + "="*80)
    print(f"    {'Test year':>9} {'Train n':>8} {'Train PnL':>10} {'Test n':>7} {'Test PnL':>9} {'Test WR':>8}")
    train_pnls = []
    ...
    for test_yr in test_years_to_check:
        ...
        tr_n = len(train); tr_pnl = sum(t["R"] for t in train)
        ts_n = len(test); ts_pnl = sum(t["R"] for t in test)
        ts_W = sum(1 for t in test if t["R"] > 0)
        ts_wr = ts_W/ts_n*100 if ts_n else 0
        cum_test_pnl += ts_pnl; cum_test_n += ts_n; cum_test_W += ts_W
        marker = " <- BAD" if ts_pnl < 0 else ""
        print(f"    {test_yr:>9d} {tr_n:>8d} {tr_pnl:>+8.1f}R {ts_n:>7d} {ts_pnl:>+7.1f}R {ts_wr:>6.1f}%{marker}")
    print()
    if cum_test_n > 0:
        print(f"  Combined OOS (2021-2026): n={cum_test_n} WR={cum_test_W/cum_test_n*100:.1f}% PnL={cum_test_pnl:+.1f}R")
    print(f"  Compare in-sample Variant C (etap_144): 513 trades / +129R / 1 bad / WR 52.6%")
