"""etap_150: Walk-forward Asia-session filter на Variant C.

etap_149: Asia session (00-07 UTC) на Variant C = 150 trades / +60.7R / 1 bad / R/tr +0.40.
Caveat: 5 sessions × 7 years = high multi-comparison bias.

Тест:
  Для каждого test year, train на prior years -> compute Asia-session edge.
  Если Asia-session edge stable across train windows, и test PnL positive
  каждый год -- promote to Variant F.
"""
...
def main():
    print("etap_150: Walk-forward Asia-session filter (00-07 UTC) на Variant C")
    print()
    all_trades = []
    ...
    # Asia subset
    asia_trades = [t for t in all_trades if 0 <= t["hour"] < 7]
    print(f"  Variant C total trades: {len(all_trades)}")
    print(f"  Asia session trades (00-07): {len(asia_trades)}")
    print()

    # Year-by-year on Asia subset
    print("  Asia-session year-by-year:")
    print("  " + "="*80)
    ...
    # Walk-forward: для каждого test year, проверить Asia-session edge на train
    print("  Walk-forward (Asia-session filter):")
    print("  " + "="*100)
    print(f"    {'Test year':>9} {'Train n':>7} {'Train PnL':>10} {'Train WR':>9} {'Train R/tr':>10}  "
          f"{'Test n':>6} {'Test PnL':>9} {'Test WR':>8}")
    cum_test_pnl = 0.0; cum_test_n = 0; cum_test_W = 0
    test_years = sorted(by_year.keys())[1:]  # skip first year
    for test_yr in test_years:
        train = [t for t in asia_trades if t["year"] < test_yr]
        test  = [t for t in asia_trades if t["year"] == test_yr]
        ...
        tr_rpt = tr_pnl/tr_n if tr_n else 0
        cum_test_pnl += ts_pnl; cum_test_n += ts_n; cum_test_W += ts_W
        marker = " <- BAD" if ts_pnl < 0 else ""
        print(f"    {test_yr:>9d} {tr_n:>7d} {tr_pnl:>+8.1f}R {tr_wr:>7.1f}% {tr_rpt:>+8.2f}  "
              f"{ts_n:>6d} {ts_pnl:>+7.1f}R {ts_wr:>6.1f}%{marker}")
    print()
    if cum_test_n > 0:
        print(f"  Asia-session OOS (2021-2026): n={cum_test_n} WR={cum_test_W/cum_test_n*100:.1f}% PnL={cum_test_pnl:+.1f}R / R/tr={cum_test_pnl/cum_test_n:+.2f}")
    print()
    # Compare: non-Asia all trades for benchmark
    non_asia = [t for t in all_trades if t["hour"] >= 7 or t["hour"] < 0]
    cum_na_pnl = sum(t["R"] for t in non_asia)
    print(f"  Reference: full Variant C: 513 trades / +129R / R/tr +0.25")
    print(f"  Reference: non-Asia subset: {len(non_asia)} trades / {cum_na_pnl:+.1f}R / R/tr {cum_na_pnl/len(non_asia):+.2f}")
