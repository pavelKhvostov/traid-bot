"""etap_153: Walk-forward Variant E (Variant C + score>=0.00).

Variant E (etap_148): 159 trades / WR 50.9% / +91.6R / 2 bad / DD -7.6R / R/tr +0.58.
"Variant C cleaned by momentum filter" -- score>=0 keeps only signal-aligned trades.

OOS validation closes the OOS matrix (B, C, D, E, F, H, Tier-1).
"""
...
def collect_variant_e_trades(symbol, start_date, cap, th, cf):
...
def main():
    print("etap_153: Walk-forward Variant E (Variant C + score>=0.00)")
    print()
    all_trades = []
    for sym, start, cap, th, cf in SYMBOLS:
        ts = collect_variant_e_trades(sym, start, cap, th, cf)
        for t in ts: t["symbol"] = sym
        all_trades.extend(ts)
    all_trades.sort(key=lambda t: t["time"])
    print(f"  Variant E total trades: {len(all_trades)}")
    print()

    print("  Variant E year-by-year:")
    print("  " + "="*80)
    ...
    print()
    if cum_test_n > 0:
        print(f"  Combined OOS 2021-2026: n={cum_test_n} WR={cum_test_W/cum_test_n*100:.1f}% PnL={cum_test_pnl:+.1f}R / R/tr={cum_test_pnl/cum_test_n:+.2f}")
    print(f"  In-sample reference: 159 / WR 50.9% / +91.6R / R/tr +0.58")
