"""Stage 1 на 1.1.2: vary entry_pct отдельно для SWEPT / NOT-SWEPT / ALL.
...
def main() -> None:
    print(f"[INFO] Stage 1 на 1.1.2: SWEPT / NOT-SWEPT / ALL — vary entry_pct")
    print()
    ...
    for label, df in [("SWEPT", df_swept), ("NOT-SWEPT", df_not_swept), ("ALL", df_all)]:
        print()
        print("=" * 100)
        print(f"Stage 1 — {label} (cache={len(cache_swept) if label=='SWEPT' else len(cache_not_swept) if label=='NOT-SWEPT' else len(cache_all)})")
        print("=" * 100)
    ...
    # Сводная таблица: best по каждой группе
    print()
    print("=" * 100)
    ...
    out = Path("signals/optimize_1_1_2_stage1_compare.csv")
