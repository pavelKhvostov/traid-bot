...
def simulate_floating(
    ...
    *,
    ...
    # Strict lookahead-safe fill_start.
    # ob_vc canon: можно открывать ордер не раньше чем закроются:
    #   1) cur HTF bar (OB-pair полностью валидна)
    #   2) LTF c3 (FVG валидна)
    #   3) Williams n=2 opposite fractal (canon condition #9)
    # scan_ob_vc_events записывает strict_detection_ts; если нет — fallback на старую формулу.
    tf_min = 15 if sig["fvg_tf"] == "15m" else 20
    strict_ts = sig.get("strict_detection_ts")
    if strict_ts is not None:
        fill_start = pd.Timestamp(strict_ts)
        if fill_start.tzinfo is None:
            fill_start = fill_start.tz_localize("UTC")
    else:
