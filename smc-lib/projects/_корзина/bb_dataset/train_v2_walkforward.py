...
Input:  ~/Desktop/bb_obvc_1h2h_v2.parquet (with 106 features + trade_label)
Output:
  bb_predictions_v2.csv     — per-zone walk-forward P_win + actual
  bb_metrics_v2.json        — per-fold metrics
  bb_feature_importance_v2.csv
  bb_strategy_filter_v2.txt — WR/RR при разных thresholds (главный результат)
...
if os.environ.get("BB_PARQUET_PATH"):
    ...
    OUT_DIR = DATA.parent
elif (_ARCHIVE_OUT / "bb_obvc_1h2h_v2.parquet").exists():
    DATA = _ARCHIVE_OUT / "bb_obvc_1h2h_v2.parquet"
    OUT_DIR = _ARCHIVE_OUT
else:
    DATA = Path.home() / "Desktop" / "bb_obvc_1h2h_v2.parquet"
...
def main():
    t0 = time.time()
    print(f"[bb-train-v2] loading {DATA}...")
    ...
    print("\n[bb-train-v2] walk-forward training...")
    res = walk_forward(df, feature_cols, test_years=1.0, train_years=4.0, retrain_months=1)

    OUT_DIR.mkdir(exist_ok=True)
    res["preds"].to_csv(OUT_DIR / "bb_predictions_v2.csv", index=False)
    print(f"\n[bb-train-v2] saved predictions → bb_predictions_v2.csv ({len(res['preds'])} rows)")

    with open(OUT_DIR / "bb_metrics_v2.json", "w") as f:
    ...
    print("\n[bb-train-v2] strategy filter analysis (KEEP if P_win >= threshold):")
    print("=" * 100)
    filter_df = filter_strategy_eval(res["preds"])
    print(filter_df.to_string(index=False))
    filter_df.to_csv(OUT_DIR / "bb_strategy_filter_v2.csv", index=False)

    # Target check: WR ≥ 60% и RR ≥ 2.2
    print("\n[bb-train-v2] TARGET CHECK (WR>=60% AND RR>=2.2):")
    ...
    print(f"\n[bb-train-v2] TOTAL: {(time.time()-t0)/60:.1f} min")
