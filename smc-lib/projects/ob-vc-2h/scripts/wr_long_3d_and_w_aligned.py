...
is BOTH 3D-aligned AND W-aligned.

D fractal qualifies if:
  - Same level as a 3D fractal, within that 3D bar's window
  - AND same level as a W fractal, within that W bar's window
...
def find_synced(d_frac, htf_frac, htf_cans, htf_ms):
    """For each HTF fractal, find matching D fractal with same level in its window.
    Returns set of D fractal indices that are synced."""
    synced = set()
    for (ih, lvl_h, _) in htf_frac:
        ts_h = int(htf_cans[ih].open_time)
        win_lo, win_hi = ts_h, ts_h + htf_ms
        for (id_, lvl_d, _) in d_frac:
            ts_d = int(cans_1d[id_].open_time)
            if win_lo <= ts_d < win_hi and abs(lvl_d - lvl_h) < 1e-6:
                synced.add(id_)
                break
    return synced


# Find D fractals synced with 3D (FL with FL, FH with FH)
sync_3d_FL = find_synced(fls_d, fls_3d, cans_3d, TF_3D_MS)
sync_3d_FH = find_synced(fhs_d, fhs_3d, cans_3d, TF_3D_MS)
sync_w_FL = find_synced(fls_d, fls_w, cans_w, TF_W_MS)
sync_w_FH = find_synced(fhs_d, fhs_w, cans_w, TF_W_MS)

inter_FL = sync_3d_FL & sync_w_FL
inter_FH = sync_3d_FH & sync_w_FH

print(f"D fractals: FL={len(fls_d)} FH={len(fhs_d)}")
print(f"3D-synced D: FL={len(sync_3d_FL)} FH={len(sync_3d_FH)}")
print(f"W-synced D:  FL={len(sync_w_FL)} FH={len(sync_w_FH)}")
print(f"\n3D ∩ W synced D: FL={len(inter_FL)} FH={len(inter_FH)}  total={len(inter_FL)+len(inter_FH)}")

# Build anchors (i+1) from intersection
inter_anchors = []
inter_idxs = list(inter_FL) + list(inter_FH)
for id_ in inter_idxs:
    if id_ + 1 < len(cans_1d):
        inter_anchors.append(int(cans_1d[id_+1].open_time))
A_TS = np.array(sorted(inter_anchors), dtype=np.int64)
print(f"3D ∩ W aligned D VWAPs (i+1): {len(A_TS)}")
