"""Hybrid v3-SKIP: a-suffix — SKIP trades where R% < 0.5% (вместо move entry).
Сохраняем natural high-WR setups, отфильтровываем micro-R.
"""
...
def tbm_run(entry, sl, direction, born_ms):
    R = abs(entry - sl)
    if R <= 0: return {"touched": False, "skipped": False}
    TP1 = entry + R if direction == "long" else entry - R
    i_start = int(np.searchsorted(ts_1m, born_ms))
    if i_start >= len(ts_1m): return {"touched": False, "skipped": False}
    ...
    if touch_rel == -1:
        return {"touched": False, "skipped": False, "R_pct": R/entry*100}
    ...
    if tp1_rel != -1 and (sl_rel == -1 or tp1_rel <= sl_rel):
        ...
        outcome = "timeout"
    return {"touched": True, "skipped": False, "outcome": outcome, "R_pct": R/entry*100}
...
for k, (key, sub) in enumerate(groups):
    ...
    if suffix == "a":
        ...
        if direction == "long":
            ...
            r_raw = (entry - sl) / entry if entry > 0 else 0
        else:
            sl = prev.high; entry = cur.high
            r_raw = (sl - entry) / entry if entry > 0 else 0
        # SKIP if R% < 0.5%
        if r_raw < R_PCT_MIN:
            n_skipped_a += 1
            records.append({"t_id": t_id, "skipped": True, "touched": False, "R_pct": r_raw*100})
            continue
    else:
        # OLD rule
        if direction == "long":
            ...
            entry = cf.fvg_zone_hi - deep * (cf.fvg_zone_hi - cf.fvg_zone_lo)
            sl = cf.drop_lo
        else:
            cf = sub.sort_values("fvg_zone_lo", ascending=True).iloc[0]
            deep = 0.8 if n_comp >= 2 else 0.2
            entry = cf.fvg_zone_lo + deep * (cf.fvg_zone_hi - cf.fvg_zone_lo)
            sl = cf.drop_hi

    out = tbm_run(entry, sl, direction, born)
    out["t_id"] = t_id
    records.append(out)

rdf = pd.DataFrame(records)
print(f"Processed: {len(rdf):,}")
print(f"a-suffix SKIPPED (R% < 0.5%): {n_skipped_a:,}\n")

T_ORDER = ["T1a","T1b","T2","T3a","T3b","T4","T5a","T5b","T6","T7a","T7b","T8",
           "T9a","T9b","T10","T11a","T11b","T12","T13a","T13b","T14","T15a","T15b","T16"]

print(f"{'T':<6} {'N_orig':>7} {'skip':>5} {'N_eff':>6} {'WR%':>6} {'EV':>9} {'ΣR':>6} {'avgR%':>7}")
print("-" * 65)
total_sum = 0
for t in T_ORDER:
    g_all = rdf[rdf.t_id == t]
    n_orig = len(g_all)
    if n_orig == 0: print(f"{t:<6} 0"); continue
    g = g_all[g_all.skipped != True]   # exclude skipped
    n_skip = int(g_all.skipped.sum())
    n_eff = len(g)
    if n_eff == 0:
        print(f"{t:<6} {n_orig:>7} {n_skip:>5} {0:>6}"); continue
    n_t = g.touched.sum()
    tg = g[g.touched]
    ...
    print(f"{t:<6} {n_orig:>7} {n_skip:>5} {n_eff:>6} {wr:>5.1f}% {ev:>+8.3f}R {total:>+5}R {avg_rpct:>6.2f}%")
print(f"\nΣ всех: {total_sum:+}R / 6y")
print(f"Total skipped: {n_skipped_a}")
