"""TBM на 24 типах с НОВЫМ правилом entry:
  LONG:  Entry = max(prev.low,  cur.low)   (= non-extreme candle's edge)
         SL    = min(prev.low,  cur.low)
  SHORT: Entry = min(prev.high, cur.high)
         SL    = max(prev.high, cur.high)

FVG больше НЕ используется для entry — только для qualification ob_vc.
"""
...
cur_types = prev_types
T_MAPPING = {}
for i, (d, sw, n) in enumerate(prev_types):
    T_MAPPING[(d, sw, n, "prev", True)]  = ORIG_PREV[i] + "a"
    T_MAPPING[(d, sw, n, "prev", False)] = ORIG_PREV[i] + "b"
...
def tbm_new(direction, prev, cur, born_ms):
    """NEW rule: Entry on non-extreme edge, SL on extreme edge."""
    if direction == "long":
        # Entry = higher of (prev.low, cur.low). SL = lower
        if prev.low < cur.low:
            entry = cur.low
            sl = prev.low
        else:
        ...
        # SHORT: Entry = lower of (prev.high, cur.high). SL = higher
...
print(f"Processing {len(groups):,} ob_vc with NEW entry rule (non-extreme edge)...")
for k, (key, sub) in enumerate(groups):
    ...
    chosen = sub.iloc[0]  # FVG не нужен для entry, just need born_ms
    out = tbm_new(direction, prev, cur, int(chosen.born_ms))
    out["t_id"] = t_id
    records.append(out)

rdf = pd.DataFrame(records)
print(f"  {len(rdf):,} / {len(groups):,}  ✓\n")
rdf.to_parquet(pathlib.Path(__file__).parent.parent / "data/tbm_2h_24_new_entry.parquet",
                index=False)
...
print(f"{'T':<6} {'N':>5} {'touch':>6} {'WR%':>6} {'EV':>10} {'Σ R / 6y':>10}")
print("-" * 50)
total_sum = 0
for t in T_ORDER:
    g = rdf[rdf.t_id == t]
    ...
    total = wins - losses
    total_sum += total
    print(f"{t:<6} {n:>5} {n_t:>6} {wr:>5.1f}% {ev:>+8.3f}R {total:>+7}R")
print(f"\nΣ всех 24: {total_sum:+}R / 6y")
