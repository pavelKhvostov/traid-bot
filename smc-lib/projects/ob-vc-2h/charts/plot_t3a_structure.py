"""T3a structure: LONG SWEPT n_FVG=1 extreme=prev wick_ratio ≥ 2× (strong wick).
NEW rule (Hybrid v2). Пример: 04-06-2026 15:00 МСК (ratio 2.21×, R%=0.30%).
"""
import sys, pathlib
...
WIN_S = int(datetime(2026, 6, 4, 11, 0, tzinfo=MSK).timestamp() * 1000)
WIN_E = int(datetime(2026, 6, 5, 12, 0, tzinfo=MSK).timestamp() * 1000)
...
PREV = dict(o=63_266.0, h=63_276.0, l=62_205.0, c=62_546.0)
CUR  = dict(o=62_546.0, h=64_244.0, l=62_392.0, c=64_230.0)
OB_ZONE = (PREV["l"], CUR["c"])
DROP_AREA = (PREV["l"], PREV["o"])

# Single FVG (n_FVG=1)
FVG = {"c1_ms": int(datetime(2026,6,4,15,0,tzinfo=MSK).timestamp()*1000),
       "c3_close_ms": int(datetime(2026,6,4,15,45,tzinfo=MSK).timestamp()*1000),
       "zone": (62_706.0, 63_149.0)}

prev_body_lo = min(PREV["o"], PREV["c"])   # 62,546
cur_body_lo  = min(CUR["o"], CUR["c"])     # 62,546
prev_wick = prev_body_lo - PREV["l"]       # 341
cur_wick  = cur_body_lo - CUR["l"]         # 154
wick_ratio = prev_wick / cur_wick          # 2.21
...
ENTRY_TOUCH_MS = int(datetime(2026, 6, 5, 6, 6, tzinfo=MSK).timestamp() * 1000)
TP1R_HIT_MS = int(datetime(2026, 6, 5, 6, 16, tzinfo=MSK).timestamp() * 1000)
...
fig.suptitle("T3a structure  —  LONG SWEPT n_FVG=1 extreme=PREV  wick_ratio ≥ 2×  (NEW rule)",
...
ax.add_patch(Rectangle((x1, FVG["zone"][0]), x2-x1, FVG["zone"][1]-FVG["zone"][0],
                        facecolor=GREEN, alpha=0.30, edgecolor=GREEN,
                        linewidth=1.0, zorder=3))
ax.text(x1, FVG["zone"][1] + 30, "Single 15m FVG (n=1)", fontsize=9,
...
ax.text(xe, ENTRY, f"  ENTRY = cur.low  ${ENTRY:,.0f}", color=GREEN,
...
ax.annotate(f"ENTRY FILL\n{to_dt(ENTRY_TOUCH_MS):%d-%m %H:%M}",
            ...
            xytext=(to_dt(ENTRY_TOUCH_MS), ENTRY - 400),
...
ax.annotate(f"TP 1.0R hit\n{to_dt(TP1R_HIT_MS):%d-%m %H:%M}\n+1R за 10 мин ✓",
            ...
            xytext=(to_dt(TP1R_HIT_MS), TPS[0][1] + 400),
...
ax.annotate(f"SL hit (post-TP1R)\n{to_dt(SL_HIT_MS):%d-%m %H:%M}\nакадем.",
            ...
            xytext=(to_dt(SL_HIT_MS), SL - 300),
            fontsize=8, color="#7d0000", ha="center",
            arrowprops=dict(arrowstyle="->", color=RED, lw=1.0), zorder=21)

for ms, lbl in [(OB_PREV_OPEN, "prev"), (OB_CUR_OPEN, "cur"), (OB_CUR_CLOSE, "close")]:
    dt = to_dt(ms)
...
info_text(0.98, "T3a", fontsize=28, fontweight="bold", color="white",
...
nums_text = (
    ...
    f"ratio         =  {wick_ratio:.2f}×  ≥ 2 ✓ → a\n\n"
    f"FVG (n=1)     =  [{FVG['zone'][0]:,.0f}; {FVG['zone'][1]:,.0f}]\n"
    f"  (не используем для entry)\n\n"
    f"Entry ${ENTRY:,.0f}  •  SL ${SL:,.0f}\n"
    f"R = ${R:,.0f} ({R/ENTRY*100:.2f}%)\n"
    f"TP1R = ${tp1r:,.0f}\n\n"
    f"Outcome:  ✅ WIN +1R за 10 мин"
)
...
out = pathlib.Path.home() / "Desktop/i-rdrb-charts/ob_vc_2h_T3a_structure.png"
