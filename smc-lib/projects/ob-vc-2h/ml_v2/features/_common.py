"""Shared utilities for v2 feature builders: 1m loader, TF aggregation, anchors."""
...
# Pruned HMA lengths (drop redundant 55, 89, 144, 233 — strongly correlated with 50, 78, 100, 200)
...
def aggregate(rows_1m: np.ndarray, tf_ms: int, anchor: int = 0) -> np.ndarray:
    """Aggregate 1m rows to TF bars. Returns (N, 6) = [open_ms, o, h, l, c, v]."""
    ...
    out[:, 5] = np.add.reduceat(rows_1m[:, 5], starts)      # volume sum
    return out
...
def find_idx_at_or_before(ts_arr: np.ndarray, target_ms: int) -> int:
    """Last bar index with open_time <= target_ms. -1 if none."""
    i = int(np.searchsorted(ts_arr, target_ms, side="right")) - 1
    return i
