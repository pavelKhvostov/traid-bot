"""v3 HONEST feature builder — fixes lookahead bug from v3.

Uses build_hma_features_at_entry_honest (INTRADAY partial-bar at entry_ms).
Same 590 HMA features + 11 wait features.

Output: features_v3_hma_honest.parquet
"""
...
OUT = REPO / "ml_v3" / "features_v3_hma_honest.parquet"


def main():
    t0 = time.time()
    print("=" * 72)
    print("v3 HONEST HMA Feature Builder (entry_fill_ms anchor + INTRADAY partial-bar)")
    print("=" * 72)
    ...
    for asset in ("BTC", "ETH"):
        ...
        t2 = time.time()
        print(f"[3/4] [{asset}] building features (HONEST INTRADAY HMA)...")
        feats = build_hma_features_at_entry_honest(bars, rows_1m, entry_subset)
        print(f"    HMA features: {len(feats)}  ({time.time()-t2:.1f}s)")
