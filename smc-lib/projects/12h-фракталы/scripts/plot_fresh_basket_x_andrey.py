"""Канонический chart: basket ∩ Andrey ML — UPDATED для 12h-fractal-new.

Источник: ~/smc-lib/scripts/plot_basket_ml_intersection_2026_expert.py
Все настройки/оформление СОХРАНЕНЫ as-is.

ТОЛЬКО изменено:
    - Источник Basket: D_stage4_combined.parquet (наш новый Basket)
    - n_C: наш n_confluent (количество B-блоков confluent)
    - Период: последние 6 месяцев до текущего момента (как канон)
"""
...
# === Expert canon palette === (как в канон-скрипте)
BULL = '#01a648'
BEAR = '#131b1b'
DOJI = '#888'
...
# ─── НОВЫЙ источник данных: D_stage4_combined (12h-fractal-new) ─────
df_m = pd.read_parquet(Path.home()/"Desktop/12h-fractal-new-out/D_stage4_combined.parquet")
df_m["dt"] = pd.to_datetime(df_m["ts_ms"], unit="ms", utc=True)
# Map direction для совместимости: short → high, long → low (канон format)
df_m["direction_canon"] = df_m["direction"].map({"short": "high", "long": "low"})
df_m["ts"] = df_m["ts_ms"]
df_m["key"] = df_m["ts"].astype(str) + "_" + df_m["direction_canon"]

# ─── Andrey hybrid signals (как в каноне) ──────────────────────
...
print(f"Andrey hybrid signals: {len(sig_sub)} (pre-filtered p_main ≥ 0.3)")

# Intersection via inner join (как в каноне)
df_join = df_m.merge(sig_sub, on="key", how="inner")
print(f"Intersection (Basket ∩ Andrey p_main ≥ 0.3): {len(df_join)}")

# Окно: последние 180 дней (как канон)
END = pd.Timestamp.now(tz="UTC")
START_WIN = END - pd.Timedelta(days=180)
df_2026 = df_join[(df_join["dt"]>=START_WIN) & (df_join["dt"]<=END)].copy()

# n_C из нашего n_confluent (количество B-блоков confluent)
df_2026["n_C"] = df_2026["n_confluent"].fillna(0).astype(int).clip(0, 9)
...
# === Расширенная история для HMA-200 и VWAP === (как в каноне)
print("Loading extended history for HMA-200 + VWAP anchors...")
...
# === VWAPs === (как в каноне)
print("Computing anchored VWAPs from D fractals...")
...
print(f"  D fractals since 2018: {len(fractals)}")
...
below = [s for s in scored if 50000 <= s['cur'] < last_close]
above = [s for s in scored if last_close < s['cur'] <= 70000]
...
print(f"  Plotted: {len(eff_below)} eff_below, {len(eff_above)} eff_above, "
      f"{len(worked_below)+len(worked_above)} worked")

# === Markers === (как в каноне)
MARKER_SIZE = 180
...
future_end = today_dt + timedelta(hours=24*12)
...
n_fh = (df_2026["direction_canon"]=="high").sum()
n_fl = (df_2026["direction_canon"]=="low").sum()
