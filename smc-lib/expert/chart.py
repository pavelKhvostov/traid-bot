...
CSV = pathlib.Path.home() / "traid-bot/data" / ASSET_CSV[ASSET]

FETCH = pathlib.Path.home() / "smc-lib/scripts/fetch_1m_missing.py"
symbol = ASSET_SYMBOL.get(ASSET, f"{ASSET}USDT")
print(f"Auto-updating 1m data for {ASSET} ({symbol})...")
res = subprocess.run([sys.executable, str(FETCH), symbol], capture_output=True, text=True, timeout=300)
for line in (res.stdout.strip().split('\n')[-3:] if res.stdout else ['(no fetch output)']):
    print(line)
if res.returncode != 0:
...
ax.yaxis.set_major_locator(MultipleLocator(y_step_for_price(rows[-1][4])))
...
y_step = y_step_for_price(current_price)
filtered = [t for t in existing_ticks if abs(t - current_price) > y_step * 0.5]
all_ticks = sorted(set(filtered + [current_price]))
ax.set_yticks(all_ticks)
labels=[]
for t in all_ticks:
    if abs(t - current_price) < 0.0001:
        labels.append(f' {current_price:,.2f} ' if current_price < 100 else f' {current_price:,.0f} ')
    else:
        labels.append(f'{t:,.2f}' if current_price < 100 else f'{int(t):,}')
...
# Используем anchor-окно последних 1100 дней (расширено 2026-06-02)
anchor_horizon = win_end - 1100*86400*1000
...
VWAP_MIN_DIST_PCT = 1.0  # min разнос между VWAPs одной стороны, %
def pick_diverse(cands, n, min_dist_pct):
    cands = sorted(cands, key=lambda x: -x['composite'])
    picked = []
    for c in cands:
        if len(picked) >= n: break
        if all(abs(c['cur_value'] - p['cur_value']) / c['cur_value'] * 100 >= min_dist_pct
               for p in picked):
            picked.append(c)
    return picked
top_below = pick_diverse(scored, 4, VWAP_MIN_DIST_PCT)
top_above = pick_diverse(scored_above, 2, VWAP_MIN_DIST_PCT)
...
# Заголовок (canon §5) — ASSET из CLI
TF_LABEL = '6h'
...
out_path = OUT / f"{ASSET.lower()}_6h_pred12h_basket_{to_dt(win_end).strftime('%Y-%m-%d')}.png"
