...
### Backtest-only (2) — в live не интегрированы

- **VIC_BOS**        — VIC-уровень + BOS на 3m (quadruple H-L-H-L). 3y BTC: WR 53.6%, +37R.
- **Strategy 1.1.1** — multi-TF nested OB+FVG: OB-D + FVG-4h → OB-{1h‖2h} + FVG-{15m‖20m}.
                       3y BTC: WR 56.5%, +12R.
...
├── strategies/
│   ├── __init__.py
│   ├── base.py           # Signal, Zone, Level (dataclass-ы), format_signal_telegram
│   ├── ob1h_core.py      # общий движок подтверждения 1h (find_first_confirmation_in_zone)
│   ├── obx4.py
│   ├── fvg.py
│   ├── ob_htf.py
│   ├── rdrb.py
...
│   ├── marubozu.py
│   ├── vic_evot.py
│   └── strategy_1_1_1.py # backtest-only
├── tests/                # pytest (test_vic_evot.py, test_vic_levels.py, conftest.py)
├── backtest_*.py         # одноразовые исследовательские скрипты
