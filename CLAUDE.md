...
0. **(ветка `pavel` / ML-направление)** Прочитай `vault/00-home/ЭТАЛОН-читать-в-начале-сессии.md` ПЕРВЫМ — текущий эталон нейросетей/ML, карта etap_174-185, инфраструктура, бот.
0b. Прочитай `vault/00-home/ПОЛНАЯ-КАРТА-ПРОЕКТА.md` — широкий контекст: live-система (main.py vs нейро-бот), вся история etap, индикаторы, SMC-каноны, что работает/не работает.
...
6. Если нужен график TradingView — прочитай `vault/00-home/TradingView-MCP-как-запускать.md` (как запустить TV с CDP-портом, баги MCP).

### Команда агентов (.claude/agents/, 7 шт)

При работе подключаются специализированные агенты с зашитым знанием проекта (чтобы не было казусов): **winrate-truth** (реальный WR с индикаторами, не занижать), **ml-validator** (обязательный shuffle-тест), **bot-vault-guardian** (бот/vault/git), smc-reviewer, spec-syncer, backtest-auditor, pitfall-checker. См. `.claude/agents/README.md`.
