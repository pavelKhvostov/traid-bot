...
# ---- Окно поиска фрактала (FRACTAL_WINDOW=4): фрактал может отстоять от FVG ----

def test_fractal_offset_k1_long(make_15m, make_1d):
    """Фрактал на 1 свечу раньше FVG-start: k=1.

    7 свечей, n=7 → pos_i=4 (FVG-start), pos_ip2=6 (last_closed).
    Фрактал на pos_f=3 (k=1, на 1 раньше pos_i).
    Для k=0 фрактал-проверка фейлится (low(4) НЕ < low(3)=99.0)."""
    df = make_15m([
...
def test_fractal_offset_beyond_window_returns_none(make_15m, make_1d):
    """Фрактал на k=4 — за пределами FRACTAL_WINDOW=4 → None.

    10 свечей, фрактал на pos_f=0 (k=6) с low ниже vic, ни одна позиция в
    окне {n-3..n-6} = {7,6,5,4} не даёт валидного фрактала."""
    df = make_15m([
        ("2026-04-27 00:00", 99,    100.5, 99,    100  ),  # касание (low<=vic)
        ("2026-04-27 00:15", 100,   100.5, 99.7,  100.2),  # обычная
        ("2026-04-27 00:30", 100.2, 100.6, 99.9,  100.4),
        ("2026-04-27 00:45", 100.4, 100.7, 100.1, 100.5),
        ("2026-04-27 01:00", 100.5, 100.8, 100.2, 100.6),  # pos=4 (k=3), low=100.2 > vic
        ("2026-04-27 01:15", 100.6, 100.9, 100.3, 100.7),  # pos=5 (k=2)
        ("2026-04-27 01:30", 100.7, 101.0, 100.4, 100.8),  # pos=6 (k=1)
        ("2026-04-27 01:45", 100.8, 101.1, 100.5, 100.9),  # pos=7 = pos_i (k=0), high=101.1
        ("2026-04-27 02:00", 100.9, 101.2, 100.6, 101.0),  # pos=8
        ("2026-04-27 02:15", 102.0, 102.5, 101.5, 102.0),  # pos=9 = i+2: low=101.5
    ])
    # FVG валиден (high(7)=101.1 < low(9)=101.5), но фрактал-окно пусто.
