"""Помесячная разбивка 1.1.2 ALL с финальным конфигом.
...
def main() -> None:
    print(f"[INFO] 1.1.2 ALL monthly breakdown:")
    print(f"       entry_pct={ENTRY_PCT}, sl_pct={SL_PCT}, RR={RR_TARGET}, no_entry=on")
    print()
    ...
    for month, sub in df_t.groupby("month"):
        ...
        rows.append({
            "month": month,
            "signals": len(sub), "no_entry": int(no_entry),
            "wins": int(wins), "losses": int(losses),
            "open": int(opens), "nf": int(nf),
            "closed": int(closed),
    ...
    weeks = DAYS_BACK / 7
    print(f"  Frequency: {len(df_t) / weeks:.2f} sig/week ({total_closed / weeks:.2f} closed/week)")

    out = Path("signals/analyze_1_1_2_all_monthly.csv")
