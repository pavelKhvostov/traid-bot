"""Проверка OBX4: зоны на 4h + OB 1h через общее ядро."""
from __future__ import annotations

from data_manager import update_df_incrementally
from strategies import obx4
...
def main() -> None:
    ...
    print(f"[OBX4] загружаем историю {symbol} 1h для ob1h-ядра...")
    df_1h_raw = update_df_incrementally(symbol, "1h")
    df_1h = to_ref_format(df_1h_raw)
    print(f"[OBX4] 1h: {len(df_1h)} свечей")
