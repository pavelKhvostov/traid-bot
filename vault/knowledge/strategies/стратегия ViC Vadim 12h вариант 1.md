...
date: 2026-05-20
status: predictor-validated / execution-KILL (2026-06-18, precision != expectancy)
...
## Бэктест ИСПОЛНЕНИЯ (2026-06-18) — KILL как ТОРГУЕМЫЙ сигнал
Закрыта открытая задача #1: формализованы entry=close(i), SL=свип-фитиль(high/low(i)+buf),
TP={RR-cap grid, time-stop i+2}, гонка SL/TP вперёд по 12h (`backtest_v1_signal_rr.py`).
**Результат — не монетизируется:**
- BTC: лучший RR=3.0 → +0.037 R/сд (+6.9R/6.4y); ETH RR=3.0 +0.151; **SOL отрицателен на ВСЕХ RR** (−0.085..−0.18).
- ВСЕ три год-нестабильны (по 3 убыточных года; плюс концентрирован в 2022; **2025-2026 минус везде**).
- Плюс только на cherry-pick RR=3 (RR1.5/2.0 в основном минус). Time-stop i+2 тонкий (BTC +0.016).
**Причина (= стена проекта):** precision 75-83% меряет, что фрактал ФОРМИРУЕТСЯ (фитиль = локальный
экстремум на ±2 бара — СТРУКТУРА), а не что цена РАЗВОРАЧИВАЕТСЯ ПРИБЫЛЬНО (направление/магнитуда =
монетка). Tight SL на фитиле + малый пост-фрактальный ход = низкий R. **Precision ≠ expectancy.**
→ Предиктор остаётся валидным как ОПИСАТЕЛЬНЫЙ флаг локального экстремума, но НЕ торгуемый сигнал
в стиле 1.1.x. Vault [[known-pitfalls]]: occurrence-predictor != tradable signal без backtest entry/SL/TP.

## Открытые задачи

1. ~~Entry / SL / TP формализация + бэктест~~ → СДЕЛАНО (2026-06-18): KILL как сделка (см. выше).
