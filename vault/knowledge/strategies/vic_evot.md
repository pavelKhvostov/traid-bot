...
2. **LL/HH-фрактал на свече f** — фрактал и FVG-start не обязаны совпадать.
   Поиск идёт от `pos_i` (= `last_closed - 2`) назад до начала day D —
   берётся ближайший к FVG валидный фрактал. Никакого жёсткого окна;
   расстояние `pos_i - f` пишется в `meta.fractal_offset_k`.
   - LONG: `low(f)` строго меньше четырёх соседей (f-2, f-1, f+1, f+2)
   - SHORT: `high(f)` строго больше четырёх соседей
   - LONG: `low(f) < maxV` (фрактал ниже уровня)
   - SHORT: `high(f) > maxV` (фрактал выше уровня)
   - Касание из п.1 случилось НЕ ПОЗЖЕ свечи f

3. **FVG между i и i+2** (`i+2 == last_closed_15m`, `i = last_closed - 2`):
   - LONG: `high(i) < low(i+2)` (есть гэп вверх)
   - SHORT: `low(i) > high(i+2)` (есть гэп вниз)
   - FVG-проверка не зависит от позиции фрактала — `i` фиксирован.
   - FVG относительно `maxV` НЕ проверяется: уровень привязан только
     через касание (п.1) и расположение фрактала (п.2). Это даёт больше
     сигналов с более низким WR на «новых» сетапах (~33% vs ~54% на
     «классических») — компромисс взят сознательно.

3a. **Структурная инвалидация:** между фракталом `f` и `last_closed` не должно
   быть противоходного фрактала.
   - LONG: ни одной свечи `g` в (f, last_closed-2], где `high(g)` строго
     больше 4 соседей (g-2, g-1, g+1, g+2). Если HH-фрактал найден —
     сигнал отвергается, потому что разворот вверх уже опровергнут.
   - SHORT: симметрично, ни одного LL-фрактала в окне.
   - Это заменяет старое жёсткое «окно фрактала из 4 позиций» на
     осмысленную проверку рыночной структуры.
...
| Фрактал есть, но low(f) > vic_level (LONG) | None |
| Фрактал есть, FVG не сформирован (high(i) ≥ low(i+2)) | None |
| FVG под уровнем (low(i+2) ≤ vic) | Signal (проверка vic-vs-FVG убрана) |
| Все 5 условий — счастливый путь LONG (k=0) | Signal с правильными полями |
| Все 5 условий — счастливый путь SHORT (k=0) | Signal с правильными полями |
| Фрактал на k=1 (1 свеча раньше FVG-start) | Signal, `fractal_offset_k=1` |
| В day D нет ни одного LL-фрактала с low<vic | None |
| LL → HH → FVG (противоход между фракталом и FVG) | None (структурная инвалидация) |
| Cross-midnight: фрактал спанит границу UTC-дня | Signal, касание только в day D |
