...
2. **LL/HH-фрактал на свече f** — фрактал и FVG-start больше не обязаны
   совпадать. Окно поиска: `f ∈ {pos_i, pos_i-1, pos_i-2, pos_i-3}`, где
   `pos_i = last_closed_15m - 2 шага` (FVG-start). Берётся первый валидный
   `f` (ближайший к FVG-start). Параметр окна — `FRACTAL_WINDOW = 4` в
   ...
   - В meta сигнала пишется `fractal_offset_k = pos_i - f` для отладки
...
| Фрактал глубже k=3 (вне окна) | None |
