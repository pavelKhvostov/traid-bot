...
MODEL_DIR = _ROOT / "research" / "elements_study" / "output" / "etap182_model"  # лучшая (книги, 2017+)
...
# etap_180: знаниевые "почему"-фичи (модель обучена с ними)
...
# etap_182: лучшая модель (LayerNorm арх, техники книг). build_book_net нужен для загрузки.
...
def load_model(device):
    ...
    # выбрать архитектуру по meta: etap_182 = book_layernorm, иначе старая ordinal
    arch = meta.get("arch", "ordinal")
    build = _e182.build_book_net if arch == "book_layernorm" else _e178.build_ordinal_net
    nets, scalers = [], []
    for fi in range(n_folds):
        net = build(len(feats)).to(device)
